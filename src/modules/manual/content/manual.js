...
export const MANUAL = [
  ...
  /* ================================================================== */
  {
    ...
    temas: [
      ...
      },
      {
        ...
        titulo: 'Ver todos los márgenes de un pantallazo',
        bloques: [
          {
            t: 'p',
            texto: 'El markup vive en la fila producto × lista, y eso está bien —es lo que permite que la misma "Mayorista 1" vaya al 30% en uno y al 50% en otro—, pero deja una pregunta sin pantalla: **"¿qué markups estoy usando?"**. Contestarla abriendo producto por producto no termina más. **Compras › Márgenes** la contesta de una.',
          },
          {
            t: 'lista',
            items: [
              '**Arriba, el resumen: los VALORES, no los productos.** Por cada lista se muestran los markups que están en uso con cuántos artículos tiene cada uno —"en Minorista, 1.240 al 35%"— y una barra para comparar de un vistazo cuál manda. Es la vista que hace corto el trabajo, justamente porque muchos productos comparten el número. Y **las excepciones saltan solas**: los tres al 62% que nadie se acuerda por qué están ahí aparecen como un renglón más, abajo de todo.',
              '**Abajo, la grilla: el "ver todos" literal.** Una fila por cosa que se vende, una columna por lista, y en cada celda el markup con el **precio final debajo** — porque un porcentaje suelto no dice si el número de la góndola cierra. El paquete fraccionado es fila propia (`↳`): tiene su costo y su markup, y aplastarlo contra el del kilo mostraría un margen que nadie cargó.',
              '**Las dos están enganchadas.** Clic en un valor del resumen y la grilla queda mostrando exactamente esos artículos, con un chip arriba para sacar el filtro. Ese movimiento es el que reemplaza al "entrar producto por producto".',
              '**Cambiar todo un grupo de una vez.** Cada valor tiene su botón *"Cambiar estos N"*, que abre la actualización masiva de márgenes **ya apuntada a ese grupo exacto**. Y exacto quiere decir exacto: si un producto está al 40% suelto y al 55% en su paquete de 500 g, cambiar "el grupo del 40%" **no toca el 55%**. El alcance viaja como las filas puntuales y no como "estos productos", que es lo que haría moverse precios que nadie pidió.',
              '**El precio puesto a mano no es un markup 0%.** Esas filas tienen su propio grupo por lista y no se pueden mover con un porcentaje: ese precio lo fijó una persona. El recuadro **"Con precio a mano"** de arriba dice cuántas son — si son muchas, el mapa de márgenes explica menos de lo que parece.',
              '**Los filtros son los mismos de Productos** (búsqueda, categoría, marca, proveedor, estado), así que todo lo de abajo se puede acotar a un proveedor o a una marca. Y el botón **Exportar CSV** baja exactamente lo que estás viendo.',
            ],
          },
          {
            t: 'nota',
            tono: 'ok',
            texto: 'No le pide NADA al servidor. Los markups ya viajaban en el snapshot del inventario que el módulo carga al entrar —se usaban solo para pintar la ficha de a un producto por vez—, así que esta pantalla es cuenta en memoria: abrirla no cuesta una consulta y cambiar un filtro tampoco. Medido con 2.700 productos y 12.150 filas de precio: **abre en 40 milisegundos**.',
          },
          {
            t: 'nota',
            tono: 'warn',
            texto: 'Muestra costos y márgenes, así que pide el mismo permiso que la ficha del producto (`compras.productos`): un cajero no la ve.',
          },
          { t: 'ruta', texto: 'Compras › Márgenes' },
        ],
      },
    ],
  },
    ...
    temas: [
      {
        ...
        titulo: 'Cargar por bultos en el ticket',
        bloques: [
          {
            t: 'p',
            texto: 'En el renglón del ticket, al lado de las unidades hay **− [bultos] +**. Antes, para vender dos cajas de 16 había que hacer la cuenta y escribir 32; ahora se aprieta **+** dos veces, o se escribe **2** directo en el campo del medio (que es lo rápido cuando son diez). **Las unidades siguen siendo la verdad**: el bulto es otra forma de escribir el mismo número, y debajo siempre se lee en palabras cuánto hay cargado.',
          },
          {
            t: 'tabla',
            cols: ['Lo que se hace', 'Lo que queda'],
            filas: [
              ['Se agrega el producto', '1 unidad — como siempre'],
              ['**+** (bulto de 16)', '**16** · *1 × bulto de 16*'],
              ['**+** otra vez', '**32** · *2 × bulto de 16*'],
              ['Se escribe **37** en unidades', '**37** · *2 bultos + 5 u*'],
              ['**+** desde 37', '**48** · *3 × bulto de 16* — sube al bulto redondo'],
              ['**−**', '**32** · *2 × bulto de 16* — baja al bulto redondo'],
              ['Se escribe **10** en bultos', '**160** — diez bultos son diez bultos, sin arrastres'],
            ],
          },
          {
            t: 'lista',
            items: [
              '**Los botones redondean, no suman sueltos.** El renglón nace en 1 unidad: si el **+** conservara ese suelto, el primer clic daría 17 en vez de 16 — y el botón existe justamente para no tener que pensar en 16. Desde una cantidad con sueltos, **+** lleva al bulto de arriba y **−** al de abajo, como cualquier escalón.',
              '**El − nunca vacía el renglón**: se apaga cuando ya está en un bulto o menos. Para sacar el artículo está la ×.',
              '**Escribir en unidades sigue valiendo para todo**: nadie está obligado a cargar bultos justos. Lo único que cambia es que ahora la pantalla dice en palabras qué es esa cantidad.',
            ],
          },
          {
            t: 'nota',
            tono: 'warn',
            texto: 'Hay **dos bultos distintos y no significan lo mismo**. El del **formato de venta** de la lista (el "Vende por 16") es una **regla**: el precio de a 16 existe porque se lleva de a 16, así que una cantidad que no da bultos justos se marca en **rojo**. El **bulto de la ficha del producto** (el que identifica el DUN) es solo una **comodidad para cargar**, y por eso en mostrador nunca se pinta de rojo: vender 5 sueltos de una caja de 10 es normal. Cuando la lista vende por N, ese número gana — manda sobre el precio.',
          },
          {
            t: 'nota',
            texto: 'El **granel no tiene bultos**: se vende por kg y los botones no aparecen. El bulto de la ficha se carga en **Compras › Productos › (el producto) · Unidades por bulto**; con 1 (o vacío) el renglón queda como siempre, sin botones.',
          },
          { t: 'ruta', texto: 'Ventas › Punto de venta (el renglón del ticket) · Compras › Productos (el bulto de la ficha) · Ventas › Formato de venta (el "Vende por N" de cada lista)' },
        ],
      },
      {
        id: 'presupuestos',
      ...
      },
      {
        ...
        bloques: [
          {
            t: 'p',
            texto: 'Al pie de **Ventas › Configuración**. Contesta una sola pregunta —**¿puedo facturar?**— y, cuando la respuesta es no, cuál de las cinco cosas falta. Está pegado al interruptor de facturación electrónica a propósito: **el interruptor es la intención** (quiero facturar) y **el panel es la capacidad** (puedo). Prender una perilla sin forma de saber si hace algo no sirve de nada.',
          },
          {
            t: 'tabla',
          ...
          },
          {
            t: 'nota',
            tono: 'info',
            texto: 'LA PRIMERA PREGUNTA CORRE AUNQUE NO HAYA CERTIFICADO. "¿ARCA está vivo?" no lleva credenciales, así que es justo la que contesta si el problema es de ellos o nuestro — y saltearla por no tener el certificado cargado sería tapar la única respuesta que se podía dar en ese estado.',
          },
          {
            t: 'nota',
            tono: 'warn',
            texto: 'SI RENOMBRÁS UN CERTIFICADO, VOLVÉ A ABRIR ESTA PANTALLA. El sistema recuerda si el archivo existe para no mirar el disco en cada venta; abrir el panel fuerza la relectura, sin reiniciar la API. Es la trampa clásica: Windows esconde las extensiones y el `.crt` que baja ARCA suele quedar como `algo.crt.crt`.',
          },
          {
            t: 'lista',
            items: [
          ...
          { t: 'ruta', texto: 'Ventas › Configuración › Facturación electrónica — diagnóstico (permiso ventas.configuracion)' },
        ],
      },
      {
        id: 'atajos',
  ...
  /* ================================================================== */
  {
    ...
    temas: [
        ...
        bloques: [
          ...
          },
          {
            ...
            filas: [
              ...
              ['Confirmar lista', 'La mercadería quedó apartada físicamente → se **arma lo que falta** (el granel se fracciona solo, lo justo para el pedido) y se **reserva ESA lista** (disponible → comprometido). Cada encargado confirma la suya'],
          ...
          },
          {
            t: 'nota',
            tono: 'ok',
            texto: '**Quién ve qué:** el fraccionador ve solo Fraccionados, el preparador solo Enteros, y quien tenga los dos permisos (o sea admin) ve las dos — necesita el pedido completo para despachar. Si el pedido no trae nada de tu lado, la pantalla lo dice ("este pedido no trae fraccionados") en vez de mostrar una tabla vacía. **Ojo: esto es una comodidad de pantalla, no un candado** — la API todavía no valida quién confirma qué lista, y no va a poder hasta que haya sesiones con token. **Confirmar arma lo que falta** (17/9/2026): si el pedido pide paquetes y en el depósito hay granel, el fraccionado lo hace el sistema al confirmar —lo justo para ese pedido, ni un paquete de más— y después reserva. Si ni el granel alcanza, no traba: baja lo preparado a lo que hay, le pone el motivo (sin pisar el que haya escrito el encargado) y avisa arriba qué renglones viajan cortos. La lista del fraccionador muestra igual el **granel suelto disponible** y un atajo a Fraccionar, para el que quiera armar de más: llega con lo que falta ya cargado y vuelve al pedido al terminar. Desconfirmar libera la reserva para seguir editando. Lo pedido y no enviado solo queda **visible** (no genera pedidos automáticos).',
          },
          {
            t: 'nota',
            tono: 'warn',
      ...
      },
      {
        id: 'ventas-sin-stock',
        actualizado: '2026-09-21',
        titulo: 'Ventas sin stock: vender igual, pero con rastro',
        bloques: [
          {
            t: 'p',
            texto: 'La caja **ya no se frena** cuando el sistema dice que no hay. Se vende, y cada rengl\u00f3n que se fue a negativo deja una incidencia en **Almac\u00e9n \u203a Incidencias \u203a Ventas sin stock** con todo lo que hace falta para averiguar qu\u00e9 pas\u00f3. La decisi\u00f3n de fondo: frenar con el cliente enfrente no arregla el stock, solo hace perder la venta \u2014 en una distribuidora la g\u00f3ndola es la verdad y el sistema va atr\u00e1s.',
          },
          {
            t: 'nota',
            tono: 'warn',
            texto: 'NO SE LLAMA "ERROR DE STOCK" A PROP\u00d3SITO. El sistema no se equivoc\u00f3: vendi\u00f3 exactamente lo que se le pidi\u00f3. Lo que cada fila dice es que **el inventario ten\u00eda MENOS de lo que hab\u00eda en la g\u00f3ndola**. Llamarlo error invita a leerlo como una falla del programa y a ignorarlo; lo que hay que hacer es ir a contar.',
          },
          {
            t: 'tabla',
            cols: ['Pieza', 'C\u00f3mo funciona'],
            filas: [
              ['**Cu\u00e1ndo nace**', 'Sola, al cobrar, cuando un rengl\u00f3n pide m\u00e1s de lo que hay. **Una por rengl\u00f3n en negativo** \u2014 no una por venta ni una por producto'],
              ['**Cu\u00e1nto anota**', 'La **diferencia**, no lo vendido: si hab\u00eda 3 y se vendieron 10, faltan **7**. Anotar la venta entera inflar\u00eda el problema'],
              ['**Qu\u00e9 guarda**', 'Producto, sucursal, **cu\u00e1nto dec\u00eda el sistema**, **cu\u00e1nto se vendi\u00f3**, la **diferencia**, el **comprobante** con su cliente, el **cajero** y la hora'],
              ['**El contador**', 'Suma al n\u00famero rojo de **Almac\u00e9n** en el men\u00fa, igual que las otras incidencias. Est\u00e1 para que moleste hasta que se cierre'],
              ['**Cerrarla contando**', '"Cont\u00e9 la g\u00f3ndola y ajusto el stock": se escribe **cu\u00e1ntas hay ahora** y el stock **queda en ese n\u00famero**. Deja un movimiento de ajuste firmado'],
              ['**Cerrarla sin tocar stock**', '"Fue un error de carga": el negativo no era de la g\u00f3ndola (el producto estaba cargado dos veces, la compra no se hab\u00eda asentado). No mueve un solo peso de stock'],
              ['**Si se anula la venta**', 'Se cierra **sola**, como `venta anulada`: la anulaci\u00f3n devuelve el stock, as\u00ed que el negativo que denunciaba dej\u00f3 de existir'],
            ],
          },
          {
            t: 'nota',
            texto: 'EL AJUSTE PONE EL STOCK EN LO CONTADO, no le suma lo que faltaba. Entre la venta y el momento de cerrar la incidencia pudo entrar mercader\u00eda (una compra, una transferencia): sumar a ciegas dejar\u00eda el n\u00famero **peor** que antes. El delta se calcula contra lo que hay en ese instante, adentro de la transacci\u00f3n.',
          },
          {
            t: 'nota',
            tono: 'info',
            texto: 'POR QU\u00c9 NO SE CIERRAN CON "LIBERAR" NI CON "MERMA". Las incidencias de siempre tienen mercader\u00eda **retenida** en cuarentena, y cerrarlas es decidir si vuelve a disponible o se da de baja. Una venta sin stock **no retuvo nada** \u2014 la mercader\u00eda ya sali\u00f3 por la puerta. Ofrecer "liberar" ah\u00ed ser\u00eda ofrecer inventar unidades que no existen, as\u00ed que esas dos resoluciones est\u00e1n bloqueadas para este tipo, en la pantalla y en el servidor.',
          },
          {
            t: 'lista',
            items: [
              '**El interruptor** est\u00e1 en Ventas \u203a Configuraci\u00f3n \u203a "Permitir vender sin stock". Apagado, el cajero vuelve a no poder cobrar hasta que alguien cargue el stock \u2014 el circuito de incidencias sigue existiendo, simplemente no se llega a usar.',
              '**Rige para todas las sucursales y todos los roles** (decisi\u00f3n del due\u00f1o, 21/9/2026).',
              '**La lista viene acotada**: todas las abiertas siempre, y las \u00faltimas 300 resueltas. Es lo que evita que la pantalla de Almac\u00e9n se vuelva m\u00e1s lenta cada mes \u2014 la historia completa queda en la base y en los reportes.',
              '**El riesgo que hay que tener presente**: el inventario en negativo descuadra la valorizaci\u00f3n y la rentabilidad **hasta que alguien cuenta**. Esto funciona si las incidencias se cierran. Si se dejan abiertas, es una fuga prolija y ordenada.',
            ],
          },
          { t: 'ruta', texto: 'Almac\u00e9n \u203a Incidencias \u203a Ventas sin stock (cerrar: admin) \u00b7 Ventas \u203a Configuraci\u00f3n (el interruptor)' },
        ],
      },
      {
        id: 'conteos',
  ...
  /* ================================================================== */
  {
    id: 'proveedores',
    titulo: 'Proveedores',
    resumen: 'La relación comercial con cada proveedor: pedidos, cuentas corrientes, echeqs, estados de cuenta y las cuentas disponibles para que los clientes les transfieran directo. Solo dueño y admin.',
    temas: [
      {
        id: 'prov-que-es',
        ...
        bloques: [
          ...
          },
          {
            ...
            filas: [
              ...
              ['**Cuentas disponibles**', 'Las cuentas bancarias de proveedores a las que los clientes transfieren directo, y cómo se van cubriendo. Cada transferencia cobra la venta y le paga al proveedor en el mismo acto'],
      ...
      {
        id: 'prov-cuentas-disponibles',
        actualizado: '2026-09-20',
        titulo: 'Cuentas disponibles: la transferencia tercerizada',
        bloques: [
          {
            t: 'p',
            texto: 'Se le debe a un proveedor. En vez de pagarle de la cuenta propia, **se le da a los clientes el alias del proveedor** y el cliente transfiere directo allá: la plata nunca pasa por el banco propio. Cada transferencia es **dos cosas a la vez** —un cobro de la venta y un pago al proveedor— y las dos nacen en la misma operación, o ninguna. La app anterior anotaba solo la segunda mitad: nunca decía QUÉ cliente había transferido ni por qué venta. Acá cada transferencia tiene su cliente y su comprobante.',
          },
          {
            t: 'flujo',
            items: ['Se carga la cuenta: proveedor, titular, alias, cuánto cubrir', 'El cajero cobra con "Transf. a proveedor"', 'Se cobra la venta y se le paga al proveedor en el mismo acto', 'La cuenta se va cubriendo hasta $0'],
          },
          {
            t: 'lista',
            items: [
              '**Cargar una cuenta** (Proveedores › Cuentas disponibles › + Nueva cuenta): proveedor, cuál de sus cuentas bancarias (o una nueva, que queda guardada en la ficha para la próxima), cuánto hay que hacerle llegar, fecha, ★ prioritaria y observación. **El titular es obligatorio**: es lo que el cliente necesita ver antes de transferir.',
              '**Cobrar** (Ventas › Punto de venta › Cobrar, o un recibo de cobranza): en Medio se elige **Transf. a proveedor** → el proveedor → la cuenta (las ★ van primero, y cada una dice cuánto le falta). Aparece **el titular en grande y el alias con "Copiar"** para dictarlo o pegarlo en el WhatsApp del cliente, y el importe se propone solo: lo que le falta a la cuenta, o lo que resta de la venta. Sirve para cualquier venta, mayorista o minorista.',
              '**Nunca de más**: una transferencia no puede superar lo que le falta a la cuenta. Si el cliente paga más que eso, se parte: el resto en otra cuenta o en otro medio ("+ Otro medio"). Y si en la vida real transfirió de más, el dueño **sube el importe** de la cuenta —queda registrado quién y cuándo—; el sistema no lo acepta solo.',
              '**El mínimo del proveedor** ("menos de $50.000 no me transfieras") se carga en su ficha (Proveedores › Proveedores › Ficha › Mínimo por transferencia). Cada transferencia lo respeta, **salvo la que cierra la cuenta**: el resto final es lo que es. Cuando lo que falta ya no llega al mínimo, la cuenta lo avisa ("resto bajo el mínimo"): solo se cubre con una transferencia por el resto exacto.',
              '**Pagado, Falta y Cant nunca se tipean**: son la suma de las transferencias vivas, y por eso no pueden desincronizarse. Los pagos tampoco se cargan a mano en esta pantalla: nacen del cobro, y eso es lo que garantiza que cada peso tenga su venta.',
              '**Dos cajas a la vez** sobre el mismo resto: el sistema las pone en fila y la segunda recibe "le faltan $X" con el número ya actualizado. No puede entrar de más aunque aprieten juntas.',
              '**El titular y el alias se congelan** con la primera transferencia: es a dónde transfirieron los clientes, y eso no cambia si el proveedor cambia de banco. Para cambiar de cuenta se corta esta y se crea otra. Una cuenta con transferencias no se borra: se **corta** (Corte = cerrarla aunque no esté completa; deja de ofrecerse en la caja y se puede reabrir).',
              '**Anular**: el pago al proveedor NO se anula desde Proveedores — muere con su venta o recibo, y ahí se anulan los dos juntos. Si ese pago ya se aplicó a una factura, la anulación se frena hasta desaplicarlo: nunca queda una mitad viva y la otra muerta.',
              '**Enviado** es el tilde de "ya le mandé el resumen al proveedor". El **Resumen** de cada cuenta (titular, alias, a cubrir, transferido, falta, y cada transferencia con su cliente y comprobante) se copia listo para WhatsApp o se imprime, y desde ahí se marca enviado.',
              '**Pagos** lista cada transferencia con su cliente, su comprobante y el cajero (con un tilde para ver también las anuladas), y **Reporte** da el total del período, la cantidad, el promedio y el desglose por proveedor — cantidad **y monto**.',
            ],
          },
          {
            t: 'nota',
            tono: 'warn',
            texto: 'Esa plata **nunca entra al banco propio**: por eso el medio es **"Transf. a proveedor"** y no "Transferencia". En el arqueo y en los reportes se ve por separado y no puede contarse como plata que entró. En la cuenta del proveedor (Estados de cuenta) el pago aparece como "Transferencia de cliente · Venta N · Cliente", sin aplicar: cuando se cargue su factura, Pagos avisa que hay plata sin aplicar, como con cualquier pago.',
          },
          { t: 'ruta', texto: 'Proveedores › Cuentas disponibles (las cuentas, los pagos, el reporte) · Ventas › Punto de venta › Cobrar › Transf. a proveedor · Ventas › Cobranzas › Nueva cobranza · Proveedores › Proveedores › Ficha (el mínimo, y las cuentas bancarias con su titular)' },
        ],
      },
    ],
  },
        ...
        bloques: [
          ...
          },
          {
            ...
            filas: [
              ...
              ['**20/9/2026**', '**CUENTAS DISPONIBLES: la transferencia tercerizada, integrada al sistema** (pedido del dueño, con las capturas de su app &ldquo;Cuentas Disponibles&rdquo;). **Qué es**: se le debe a un proveedor y, en vez de pagarle de la cuenta propia, se le da a los clientes el alias del proveedor; el cliente transfiere directo allá y esa plata nunca pasa por el banco propio. **El problema de la app anterior** era que anotaba la mitad de cada transferencia: el pago al proveedor, pero nunca qué cliente había transferido ni por qué venta — dos verdades separadas que nadie cruzaba. Acá es **un medio de pago del POS y del recibo** (&ldquo;Transf. a proveedor&rdquo;): al cobrar se elige el proveedor y cuál de sus cuentas, la pantalla muestra el titular en grande y el alias con Copiar, y al confirmar nacen EN LA MISMA TRANSACCIÓN el cobro de la venta y su espejo en `proveedor_pagos` (concepto &ldquo;Transferencia de cliente · Venta N · Cliente&rdquo;): la deuda del cliente y la del proveedor bajan en el mismo instante, o ninguna. Por eso el estado de cuenta del proveedor lo muestra al toque y, al cargar su factura, Pagos avisa que hay plata sin aplicar, como con cualquier pago. **Las reglas, con dinero de por medio**: (1) nunca de más — un pago no supera lo que le falta a la cuenta; si el cliente transfirió de más, el dueño sube el importe de la cuenta, acción explícita con nombre y hora; (2) **el mínimo del proveedor** (&ldquo;menos de $50.000 no&rdquo;, nuevo campo de la ficha) lo respeta cada transferencia SALVO la que cierra la cuenta — el resto final es lo que es; (3) la cuenta se toma con candado (`FOR UPDATE`) antes de sumar: dos cajas sobre el mismo resto se ponen en fila y la segunda recibe &ldquo;le faltan $X&rdquo; con el número ya actualizado; (4) `pagado`, `falta` y `cant` no se guardan, se SUMAN de los pagos vivos — un número que no existe como dato no puede desincronizarse; (5) el titular y el alias se congelan en la cuenta al crearla (es a dónde transfirieron los clientes); (6) los pagos no se cargan a mano: nacen del cobro, y apuntan a `venta_pagos` o a `cobranza_pagos` —una y solo una, con CHECK en la base— y a su espejo en `proveedor_pagos` (índice único: un pago del proveedor es espejo de UNA transferencia); (7) ese espejo **no se anula desde Proveedores**: muere con su venta o recibo, y ahí se anulan los dos juntos — y si ya se aplicó a una factura, la anulación se frena hasta desaplicarlo. **Migración 0095**: el medio `transferencia_proveedor` (distinto de `transferencia` a propósito: esa plata no entró al banco y no puede contarse como si hubiera entrado), el mínimo en `proveedores`, el **titular** en `proveedor_cuentas` (que no existía: &ldquo;3 Arroyos&rdquo; cobra en la cuenta de una persona), las tablas `cuentas_disponibles` y `cuenta_disponible_pagos`, la llave `proveedores.cuentas` para admin y el medio habilitado de entrada en la configuración de Ventas. De paso, guardar las cuentas bancarias de la ficha pasó de borrar-y-recrear a un UPSERT por alias, porque las cuentas disponibles las referencian por id. **La pantalla** (Proveedores › Cuentas disponibles) tiene las vistas de la app original: Cuentas (fecha, proveedor, titular, alias con copiar, a cubrir, pagado, falta, cant, ★, Enviado, Corte; verde al cubrirse, aviso &ldquo;resto bajo el mínimo&rdquo;), Pagos (ahora **con el cliente, el comprobante y el cajero**, y las anuladas a pedido), Reporte (total, cantidad, promedio y por proveedor cantidad **y monto** — la app vieja solo daba la cantidad) y el Resumen para mandarle al proveedor, que se copia listo para WhatsApp o se imprime y se marca enviado desde ahí. El selector de la caja es UNO para el POS y el recibo. Las cuentas abiertas se piden frescas al abrir cada cobro, porque el resto cambia con cada venta de cualquier sucursal, y el rechazo se ve ANTES de apretar (misma regla que la API, que igual revalida). **Verificado**: 45 comprobaciones de servicio sobre la base local con datos ZZ_ (alta, mínimo, sobre-cobro, partido en dos cuentas, dos cobros simultáneos por el último resto —entra exactamente uno—, candado en Pagos, anulación conjunta, anulación frenada por pago aplicado, corte, edición y borrado, listados y reporte contra la cuenta del proveedor) y el circuito real en pantalla: ficha con mínimo y cuenta con titular → cuenta disponible de $120.000 → venta en el POS de $100.000 cobrada con Transf. a proveedor (la cuenta queda en $20.000 de falta con su aviso, el ticket dice el medio y la referencia proveedor · titular · alias, Pagos y Reporte lo muestran) → anulada desde Ventas: el pago del proveedor quedó anulado con el motivo, la cuenta volvió a $120.000 y el stock a su lugar. Todo ZZ_ borrado; la base quedó como estaba.'],
              ['**18/9/2026**', '**EL LOGO DE SABOR Y AROMA, en el menú y en la entrada** (pedido del dueño, que pasó los archivos). En el sidebar reemplaza al cuadrado con las letras &ldquo;SA&rdquo;: esa caja existía porque dos letras sueltas no se leen como una marca, y con el logo de verdad sobra — el degradado que tenía además metía dos colores más peleándole al verde del menú. La versión blanca apoya directo sobre el fondo oscuro, que es para lo que está hecha. En la entrada va **arriba del título y centrado**, que es el orden en que se lee una portada: al costado competiría por el ancho y habría que achicar el logo o el nombre. **LOS ARCHIVOS QUE LLEGARON NO SERVÍAN TAL CUAL**, y conviene saberlo por si hay que rehacerlos: el **blanco venía VACÍO** —443 bytes, cero trazos, la vectorización no capturó nada, que es lo que pasa al trazar blanco sobre fondo transparente— y el **verde traía el color declarado en negro**, así que se habría visto negro. **Se usa UN SOLO ARCHIVO para los dos lugares.** El dibujo es una silueta de un color: dos SVG serían 23 KB de trazos idénticos repetidos, y dos archivos que hay que acordarse de cambiar juntos el día que el logo cambie. Va uno, en el verde de marca, y donde hace falta blanco lo pinta el CSS con `brightness(0) invert(1)` — que da blanco PURO, no &ldquo;casi blanco&rdquo;, y lo hace el navegador sin descargar nada más. Va como `<img>` y no incrustado en el código: así es un archivo aparte que el navegador cachea con su hash y **no se vuelve a interpretar en cada arranque** —son 10 KB comprimidos de puros trazos—, mientras que incrustado viajaría dentro del bundle todas las veces. Verificado en pantalla: en el menú entra con aire (38 px en una barra de 64) y en la entrada ocupa el 24% del ancho de la tarjeta. Se borró la regla de CSS del cuadrado viejo, que quedó sin uso.'],
              ['**18/9/2026**', '**LA FIRMA EN UNA SOLA FILA, y la pantalla de entrada se hace de teclado** (cuatro pedidos del dueño, con capturas). **UNO — la firma del sidebar se partía en dos.** El diseño está pensado para una barra de 1.400 px y el sidebar mide **264**: descontado el margen quedan unos 236 para &ldquo;Coftech | Soluciones digitales&rdquo; más el año, y con el tamaño original eso no entra ni cerca — el año quedaba colgado en una segunda línea. Se escalaron los tamaños (la marca a 1,15 rem) **sin tocar ni un color ni la partición del fondo**, y ahora entra todo en una línea, que es como está pensado. El lema es lo único que puede achicarse: si el sidebar se angostara más se corta con puntos suspensivos, antes que empujar el año abajo. Medido: la franja pasó de 80 px de alto a **45**, sin desborde. **DOS — &ldquo;by Coftech&rdquo; en la entrada, más chico.** Iba dentro del título, del mismo tamaño que el nombre del sistema, y eran dos cosas compitiendo. Ahora va al lado, a ~12,5 px, alineado a la **misma base** (no centrado, que lo dejaría flotando), con el `letter-spacing` abierto y el celeste solo en &ldquo;Cof&rdquo;: la marca se reconoce por el color, no por el tamaño. **TRES — el usuario se escribe, ya no se busca en una lista.** Era un desplegable: con el equipo creciendo, entrar significaba abrirlo y recorrerlo con la vista. Ahora se tipean dos letras y queda uno solo. **CUATRO — el Enter llega hasta el final.** El primer paso ya era un formulario, pero la **confirmación no**: se llegaba con Enter y ahí había que soltar el teclado y buscar el mouse, en la pantalla que se abre en cada turno. Ahora también es un formulario, con el botón &ldquo;Sí, entrar&rdquo; ya enfocado. **Un detalle que costó encontrar**: el foco inicial en Usuario no funcionaba con `autoFocus`, porque mientras la lista de usuarios no llegó el campo está **deshabilitado** —y un campo deshabilitado no toma foco—; para cuando se habilitaba, el momento del `autoFocus` (el montaje) ya había pasado. Se resolvió enfocándolo cuando los usuarios llegan, y solo si todavía no hay uno elegido: a quien volvió desde la confirmación, robarle el foco lo mandaría a empezar de nuevo. **La confirmación no se saltea**: es un paso de una tecla, y lo que viene después queda registrado a nombre de quien entró y en esa sucursal. Verificado de punta a punta: la pantalla abre con el cursor en Usuario, &ldquo;luc&rdquo; deja solo a Lucas, Enter lo elige, y el formulario entra hasta el dashboard sin tocar el mouse.'],
              ['**18/9/2026**', '**EL SISTEMA PASA A LLAMARSE &ldquo;ERP Sabor y Aroma by Coftech&rdquo;, y la firma va en el sidebar y en la tienda** (pedido del dueño). El nombre se guarda **en dos piezas y no en dos constantes**: `name` es el producto (&ldquo;ERP Sabor y Aroma&rdquo;) y `nombreCompleto` se COMPONE agregándole la firma. El motivo es concreto: el sidebar mide **264 px** y el nombre entero se cortaría con puntos suspensivos justo en &ldquo;by Coftech&rdquo;, así que arriba va el producto y la firma vive abajo, completa y con su enlace. Componer en vez de duplicar evita que cambiar uno deje el otro desfasado. El nombre completo aparece en la **pestaña del navegador** y en la **pantalla de entrada**. **LA FIRMA NO SE INTEGRÓ COMO DECÍA EL INSTRUCTIVO**, y la diferencia importa. El instructivo proponía un `<script type="text/babel">` traído de saboryaroma.com; eso funciona en una página suelta, pero acá habría costado **en cada carga**: **Babel standalone (~1,5 MB)** cuyo único trabajo sería compilar veinte líneas mientras el usuario mira una pantalla vacía, una **petición a otro dominio en el camino crítico** —si ese dominio tarda, se lo come el CRM entero— y la familia **Nunito Sans** completa para escribir una palabra. En una landing no se nota; en la caja, que se abre y se recarga cien veces por día, sí. Va compilada con el resto: **+1,3 KB de JavaScript y +0,4 KB de CSS, comprimidos, más la fuente: 31 KB de Nunito Sans servidos desde el propio dominio —el subconjunto latino, variable de 400 a 700— con `font-display: swap`, así que el menú se pinta al instante y la letra cambia sola cuando llega. Cero peticiones a Google.** El **hover es CSS y no estado de React**: el componente original guardaba `isHovered` en un `useState`, o sea un re-render por cada entrada y salida del mouse sobre algo que está SIEMPRE en pantalla; el efecto es idéntico y no cuesta nada. En la tienda eso además la deja **renderizarse en el servidor**: viaja dentro del HTML que ya se estaba mandando, sin sumar JavaScript a la métrica que Google mira para posicionarla. La tipografía es la MISMA del diseño y viaja con cada repositorio: se descartó `next/font/google` en la tienda —que es lo idiomático— porque baja la fuente EN CADA BUILD, y un deploy no puede depender de que gstatic conteste. El enlace abre **WhatsApp** y el año se calcula una vez por carga. **La tienda sigue llamándose &ldquo;Sabor y Aroma&rdquo;**: el nombre nuevo es el del SISTEMA interno, no el de la marca de cara al cliente.'],
              ['**18/9/2026**', '**UN EMPUJÓN AL MAYORISTA, en el lugar donde antes no había nada útil** (pedido del dueño: *&ldquo;un texto delicado, atractivo para el cajero&rdquo;*). El precio mayorista **por monto** lo habilita un umbral que se configura en Ventas › Configuración, y hasta ahora el cajero se enteraba **solo cuando el ticket ya lo había superado** —ahí aparece la sugerencia con su botón—, o sea **justo cuando ya no hay nada que hacer**. El momento en que sirve decirlo es ANTES: con el cliente enfrente y el changuito a mitad de camino. Entre el buscador y el ticket —el hueco que dejó el visor— va ahora una franja fina: *&ldquo;Incentivá al cliente para **compra mayorista**: le falta **$6.768,00**&rdquo;*, con una **barra de tres píxeles** que dice de un vistazo si está cerca o recién pasó la mitad; esa es la diferencia entre insistir y no insistir. **Los dos bordes son a propósito.** Aparece pasada la **mitad** del mínimo: antes de eso, &ldquo;te falta el 80%&rdquo; no es un empujón, es un número desalentador que además ocuparía pantalla en cada ticket chico. Y **desaparece al llegar**, porque ahí ya no falta nada y manda la sugerencia, que tiene el botón para aplicar — dos carteles diciendo cosas distintas sobre lo mismo es peor que ninguno. **No se inventó ningún dato ni hace falta configurar nada nuevo**: sale del mismo `montoMinimoMayorista` que ya alimenta la sugerencia. Si el umbral no está cargado, o el ticket está vacío, no se muestra nada. **El tono es deliberado**: tinte cálido, sin borde duro, texto de 13,5 px y nada de rojo — no es un error, es una oportunidad, y un cartel que grita deja de verse a los tres tickets. Seis pruebas nuevas sobre los bordes (la mitad justa habla, un peso antes no; al llegar se calla; sin umbral no molesta) y verificado en pantalla: a $23.232 de $30.000 apareció pidiendo $6.768, y a $34.848 se fue solo.'],
              ['**18/9/2026**', '**SE FUE EL VISOR DEL ÚLTIMO ARTÍCULO, y el selector de modalidad ahora dice cuál está puesta** (dos pedidos del dueño). **UNO — el visor.** Arriba del ticket había una franja que repetía EN GRANDE el último renglón cargado: nombre, cantidad, precio e importe. *&ldquo;No cumple ninguna función&rdquo;* — y es cierto: decía lo mismo que la fila que está dos centímetros más abajo, resaltada, y se comía el lugar más valioso de la pantalla para no agregar nada. Se fue la franja y **todo lo que colgaba de ella**: el estado del destello, sus cuatro llamadas y las seis reglas de CSS con su animación. El renglón recién tocado se sigue marcando en la tabla, que es donde se lo puede corregir. **DOS — el selector de modalidad.** Aplicaba bien pero volvía a decir *&ldquo;Aplicar modalidad…&rdquo;*, así que no había **dónde leer con qué precios se está vendiendo**: había que mirar renglón por renglón. Ahora el selector muestra **la modalidad que tiene el ticket**, y apretarlo es para cambiarla. Son **dos niveles y no uno**, porque el selector aplica MODALIDAD y el renglón guarda LISTA: &ldquo;Mayorista&rdquo; puede ser la 1 en un artículo y la 2 en otro, así que exigir la misma lista exacta diría &ldquo;mezclado&rdquo; en tickets que están perfectamente uniformes. El selector muestra la **modalidad común**, y **solo cuando además todos comparten la misma lista** aparece al lado el dato fino: *&ldquo;todo en Mayorista 1 · Mayorista&rdquo;*. Si se mezclan, el detalle desaparece y queda la modalidad — que sigue siendo verdad. **Sale del TICKET, no de lo último que se tocó**: da igual si la modalidad se aplicó a mano o si cada renglón cayó ahí solo por su mínimo, por el cliente o por una regla de marca. Un ticket vacío no muestra nada: no está &ldquo;en mostrador&rdquo;, no está en ninguna. Verificado en pantalla con dos artículos y dos listas: uniformes muestra las dos cosas, mezclados se cae solo el detalle.'],
              ['**18/9/2026**', '**&ldquo;VENDE POR 12&rdquo; AHORA TAMBIÉN VALE EN LA CAJA, y el formato de venta dice qué IVA usa** (dos pedidos del dueño, con capturas). **UNO — el bulto.** El formato de venta dice en cuántas unidades se vende cada lista, y ese número se respetaba **por un solo camino**: escanear el código de la caja. Eligiendo la lista **a mano** —que es como se hace cuando el cliente pide por mayor— la cantidad quedaba como estaba: se cobraba **UNA unidad al precio de la caja**. En la captura del dueño: cantidad 1, lista Mayorista, $2.462. El precio de a 12 existe PORQUE se lleva de a 12; darlo por una sola es regalar la diferencia. Ahora elegir una lista que vende de a N **sube la cantidad al múltiplo de arriba** (1 → 12, 15 → 24) y nunca baja: el que ya cargó 24 se queda con 24. Debajo del casillero el renglón dice **&ldquo;1 × bulto de 12&rdquo;**, y si alguien tipea una cantidad que no cierra, **&ldquo;no es bulto de 12&rdquo;** en rojo — avisa, no bloquea: nadie le va a impedir a la cajera vender 15 si el cliente se lleva 15, pero el renglón tiene que decir que esa cantidad no es un bulto. Solo se acomoda en el cambio **MANUAL**: cuando la lista la elige el motor (por mínimo, por cliente, por regla de marca) la cantidad no se toca, porque ahí el que manda es el mínimo configurado. El granel queda afuera: se vende por peso. **DOS — el IVA, dicho y no sobreentendido.** Los dos precios de cada fila son FINALES (con IVA adentro, como el cartel) pero el **markup se calcula sobre el NETO**, así que un 55% no cerraba contra ningún número visible y parecía que la cuenta estaba mal. Ahora el modal dice arriba **de qué IVA se trata según la ficha del producto** (21%, 10,5% o exento) y aclara que el costo y el markup son netos mientras que los precios son finales; y debajo de cada importe va **&ldquo;IVA 21% incluido · neto $1.500,00&rdquo;**, que es el número contra el que se puede verificar el markup a mano. Tres pruebas nuevas y verificado en pantalla: 1 → 12 al elegir mayorista, el aviso en rojo con 15, y el desglose del IVA en las dos filas.'],
              ['**18/9/2026**', '**ESCANEAR LA CAJA YA NO DICE &ldquo;Unidad&rdquo;, y el buscador sigue listando UN producto por fila.** Un artículo que se vende de a 12 aparecía diciendo **&ldquo;Unidad&rdquo;**, porque ese texto sale del TIPO del producto (entero o granel) y el bulto no es un tipo: es una fila del **formato de venta** con `unidades: 12`. Escanear el EAN de la caja **siempre** cargó las 12 y fijó la lista mayorista — lo que no acompañaba era lo que se leía: la fila decía *&ldquo;Unidad · $1.815&rdquo;* mientras por dentro entraban doce al precio mayorista. Hacía lo correcto y se explicaba mal. Ahora esa fila dice **&ldquo;Bulto × 12&rdquo;** y muestra el precio del **bulto cerrado**. **LA VUELTA ATRÁS DEL MISMO DÍA, y vale anotarla**: el primer intento abrió en el buscador **una fila por cada formato** —el mismo alfajor repetido, una vez suelto y otra como &ldquo;Bulto × 12 · Mayorista 1&rdquo;—. El dueño lo frenó en producción apenas lo vio: *&ldquo;acá me tiene que salir solamente EL PRODUCTO, sin su lista de precios&rdquo;*. Tenía razón y el motivo es de oficio: **el buscador contesta CUÁL artículo, no a qué precio**. Con cinco productos que coincidan y dos listas cada uno, la pantalla se llena de duplicados y **esconde a los demás productos**, que es justo lo que el cajero está buscando; la lista se elige después, en el renglón del ticket, que es donde vive esa decisión y donde ya había un selector. Quedó **una fila por producto**, como antes. **Del lado de la API viaja un dato nuevo, `precioFormato`**: el precio del bulto ENTERO y no el unitario multiplicado. Con un bulto fijado en $10.000 el unitario derivado es $833,33, y multiplicarlo por 12 en la pantalla daría **$9.999,96** — un cartel contradiciendo al que lo puso. **Pendiente conocido, anterior a esto**: el ticket trabaja con precios unitarios de dos decimales, así que ese mismo bulto de $10.000 cierra en **$10.000,07**. No lo trajo este cambio (escanear la caja daba lo mismo antes). Verificado en pantalla las dos veces, con datos ZZ_ después borrados.'],
              ['**17/9/2026**', '**CONFIRMAR LA LISTA AHORA ARMA LO QUE FALTA: se terminó el "Stock insuficiente" que trababa cada pedido de fraccionados** (pedido del dueño, mirando un TR real). El síntoma era este: pedido de 2 paquetes de 150 g y 3 de 1 kg, el depósito con **50 kg a granel y CERO paquetes**, y Confirmar lista rebotando con *"Stock insuficiente para confirmar"* en los cinco renglones. **No era escasez: era la FORMA de la mercadería.** Granel y paquete son dos cosas distintas para el stock, y el paquete no existe hasta que alguien lo arma. El sistema sabía los kilos que había y los paquetes que faltaban —los dos números están en la misma fila de la pantalla— y aun así mandaba al fraccionador a entrar producto por producto, con el modal **abierto en cero**, a tipear a mano una cuenta ya hecha. Peor todavía: ese modal **reemplazaba** al del pedido (la pantalla muestra uno por vez) y al terminar cerraba todo y lo dejaba en el listado, sin el pedido y sin saber si el fraccionado había quedado — quedaba, lo que no quedaba era el camino de vuelta. **Ahora Confirmar hace el trabajo**: fracciona lo justo para cubrir cada renglón (ni un paquete de más: el granel envasado no se desarma solo) y recién entonces reserva, todo en la **misma transacción**, así que si algo falla no queda un fraccionado huérfano. Si ni el granel alcanza **no traba**: baja lo preparado a lo que realmente hay, escribe el motivo —**respetando el que haya puesto el encargado**, que él sabe por qué falta— y el pedido sigue. Arriba de la lista queda un cartel con las dos cosas que hay que revisar: qué se armó y qué viaja corto. Dos presentaciones del mismo producto comen del mismo granel y se sirven **en orden**: la primera se lleva lo que hay. El link Fraccionar a mano sigue estando para el que quiera adelantar trabajo, pero ahora **llega con lo que falta cargado y vuelve al pedido**. Probado con cinco casos automáticos contra la base real (fracciona y reserva exacto; recorta cuando falta; no pisa el motivo escrito a mano; dos presentaciones compitiendo por el mismo granel; y confirmar de nuevo cuando ya estaba todo **no fracciona de más**) y verificado en pantalla de punta a punta, incluido el despacho con un renglón recortado. Datos de prueba borrados.'],
              ['**17/9/2026**', '**EL "Failed to fetch" DE COMPRAS Y ALMACÉN: encontrado, reproducido y cerrado por los dos lados.** El dueño mandó tres capturas del mismo instante: Compras y Almacén con *"No se pudo cargar la información desde la API: Failed to fetch"* y Ventas andando perfecto. No era azar: **las dos que fallaban bajan el catálogo entero (10 MB) y la que andaba baja 2 KB**. Reproducido con un servidor que corta la respuesta a mitad del cuerpo: el navegador tira exactamente `TypeError: Failed to fetch`, y ese error salía por un agujero del cliente HTTP — `fetch` resuelve apenas llegan los encabezados, el cuerpo sigue viajando, y si se cortaba ahí el error escapaba **sin la marca `sinRespuesta`, así que el reintento nunca se disparaba** y el texto crudo en inglés iba a la pantalla. De paso, el plazo de 20 segundos seguía corriendo mientras se bajaban y parseaban esos 10 MB, y en la máquina del mostrador eso mataba descargas sanas. **Los dos arreglos van en `httpClient`**: un cuerpo cortado ahora cuenta como respuesta que no llegó (se reintenta hasta dos veces, solo en lecturas), y el reloj se detiene cuando el servidor empieza a contestar. Probado: cortando dos veces seguidas, se recupera solo al tercer intento en 1,5 s sin mostrar nada; cortando siempre, tres intentos y un mensaje en castellano. **Y LA RAÍZ, que era el tamaño, también:** el snapshot del inventario se partió en TRES partes por velocidad de cambio — `base` (chico), `catalogo` (8 MB, cambia pocas veces por día) y `stock` (3 MB, cambia con cada venta) —, cada una con una **versión que lleva la base de datos** (secuencias de Postgres avanzadas por triggers en cada escritura, migración 0094: ningún camino nuevo puede olvidarse de avisar, y `nextval()` no bloquea a nadie). El servidor lee la versión ANTES de armar nada y contesta **304 sin cuerpo** si el CRM ya la tiene; el navegador guarda cada parte en su caché HTTP y la reutiliza —también después de un F5. Resultado medido con 2.700 productos: un refresco sin cambios pasa de bajar 10 MB a **0 bytes** (las tres partes en 10, 35 y 42 ms), y después de una venta se vuelve a bajar el stock y nada más. `GET /bootstrap` a secas sigue existiendo para el CRM que no recargó tras el deploy.'],
              ['**16/9/2026**', '**NACIÓ EL MAPA DE MÁRGENES: todos los markups de un pantallazo** (pedido del dueño: *"para ver qué markup tiene cada producto tengo que entrar producto por producto y no termino más, siendo que muchos comparten el mismo"*). **Compras › Márgenes** son dos miradas del mismo dato, enganchadas: arriba el **resumen por VALOR** —"en Minorista, 1.240 artículos al 35%", con barra para ver cuál manda y las excepciones asomando solas abajo de todo— y abajo la **grilla producto × lista** con el markup y el precio final en cada celda. Clic en un valor y la grilla queda mostrando esos artículos. Cada valor trae además **"Cambiar estos N"**, que abre la masiva de márgenes **apuntada al grupo exacto**: si un producto está al 40% suelto y al 55% en su paquete de 500 g, cambiar el 40% NO toca el 55% —el alcance viaja como las filas puntuales, no como "estos productos"—, y eso quedó escrito como prueba porque es lo que mueve precios en la góndola. El precio puesto a mano tiene su propio grupo y ningún porcentaje lo pisa. **No le pide nada al servidor**: los markups ya viajaban en el snapshot del inventario, así que es cuenta en memoria — con 2.700 productos y 12.150 filas de precio, la pantalla **abre en 40 ms** (la de Productos tarda 19). Probado de punta a punta con ese volumen: el grupo de 1.350 paquetes se cambió de 40% a 42% y los 2.700 del 35% quedaron intactos. Guía: "Formato de Venta › Ver todos los márgenes de un pantallazo"'],
              ['**16/9/2026**', '**La contraseña puede ser un PIN de 4 dígitos otra vez** (pedido del dueño: se tipea en el mostrador, de parado y con un cliente esperando). El mínimo había subido a 8 el 14/8 y vuelve a **4**; rige solo para contraseñas NUEVAS, así que nadie tiene que cambiar la suya. **Y no vuelve solo:** con 4 dígitos son 10.000 combinaciones, y el freno del login tenía una espera FIJA de 5 minutos — 1.440 pruebas por día desde una sola IP, o sea que las 10.000 caían en menos de una semana. Ahora **la espera crece en cada tanda de fallos** (5 minutos, 15, 45, y de ahí en más una hora) y **se recuerda seis horas**, así que esperar a que venza el castigo ya no devuelve el contador a cero: esa misma IP baja a unos 120 intentos por día y el ataque pasa de días a meses. El que se equivoca dos veces y entra a la tercera no se entera de que esto existe, porque entrar bien borra la escalera. Las dos piezas van juntas y así quedó escrito en el código. **Lo que sigue sin cubrir, dicho de frente**: un atacante con muchas IPs arranca de cero en cada una, y si alguna vez se filtrara la base un PIN de 4 dígitos cae en minutos — para la cuenta del superadmin, que puede TODO, conviene igual una contraseña larga.'],
              ...
              ['**27/8/2026**', '**El importador de catálogos ahora LEE el proveedor del archivo y pregunta cuando no lo encuentra** (pedido del dueño: el sistema viejo escribe el mismo proveedor con nombres levemente distintos — "BAVOSI SA" vs "BAVOSI S.A." — y elegir mal o crear un duplicado en silencio era el riesgo). El CSV de formatos de compra trae la columna `Proveedor`: **si el nombre matchea el padrón** (sin importar mayúsculas, acentos ni puntuación), el paso 2 lo **preselecciona** y lo dice ("Detectado del archivo"). **Si no está tal cual**, aparece la pregunta: *"¿es uno que ya está cargado con otro nombre, o es un proveedor nuevo?"* — con los **parecidos** como candidatos (el mismo detector de parecidos del importador del padrón: prefijo común o palabras contenidas) y el desplegable ya parado en el más probable. Elegir "nuevo" **crea el proveedor recién al confirmar la importación** (cancelar la vista previa no deja nada colgado) con el nombre del archivo y la marca de mercadería; la ficha se completa después en Proveedores. Yapa: si el export vino mezclado con filas de varios proveedores, lo avisa con el detalle. Verificado en pantalla con un CSV cuyo proveedor difería en una "S.A." de uno real: detectó, ofreció el parecido, y por la rama "nuevo" creó el proveedor e importó a su nombre (datos de prueba borrados).'],
              ['**27/8/2026**', '**El padrón de Proveedores ganó paginación** (pedido del dueño): la lista de Proveedores › Proveedores pagina de a 20 con el mismo paginador del resto de las tablas (rango, filas por página y flechas); el buscador y el filtro de migración pendiente vuelven a la página 1 al cambiar.'],
              ['**27/8/2026**', '**NACIÓ EL "USO EXCLUSIVO DE CAFETERÍA": la mercadería de Coffit que no va al mostrador** (migración 0089, pedido del dueño: productos de La Serenísima que la distribuidora compra y guarda SOLO para la cafetería, y que no se pueden vender "a menos que se decida lo contrario"). **Cómo funciona**: un tilde en la ficha del producto (Compras › Productos) lo marca como exclusivo. Sigue siendo stock normal en todo — compra por factura, existencias, conteos, vencimientos, transferencias y el envío de Almacén › Cafetería, que es su único camino de salida — pero **no se vende**: los buscadores del POS lo muestran con la marca "solo Cafetería" (esconderlo haría que un código escaneado "no exista", que parece un error) y al intentar agregarlo al ticket el rechazo dice el porqué. **El candado de verdad está en la API** (`validarEstadoVendible`, el mismo del archivado, en los dos caminos del confirm): el catálogo del POS se cachea al abrir la caja, así que un producto marcado a mitad del turno seguiría en la pantalla del cajero — el servidor lo rechaza igual. El paquete fraccionado hereda la exclusividad de la madre. La pastilla **"Cafetería"** se ve en todos los listados que muestran el tipo (Productos, Existencias, la ficha) y en el armado del envío. **El destilde lo vuelve vendible al toque**, y la ficha avisa antes de guardar: si nunca se vendió, seguramente no tiene precio cargado — cargarlo en Formato de venta antes de mandarlo a la góndola. Marcar los productos exclusivos existentes queda a mano del dueño (son pocos).'],
              ['**27/8/2026**', '**"Venta fraccionada" en un producto entero: la etiqueta mentía y se corrigió en pantalla** (pregunta del dueño mirando el historial: "¿por qué dice venta fraccionada si es un producto entero?"). La causa: la API solo distingue dos tipos de venta — granel suelto (`venta_granel`) y **todo lo demás** (`venta_fraccionada`), herencia de cuando el catálogo era granel y sus paquetes; cuando llegaron los productos **enteros** (la lata, el frasco) cayeron en el balde de "fraccionada" sin haber pasado por la fraccionadora. El movimiento en sí siempre estuvo bien (cantidad, signo, stock): mentía solo el cartel. **El arreglo (opción de pantalla, elegida por el dueño)**: donde el tipo guardado es `venta_fraccionada` pero el producto es entero, ahora se muestra **"Venta por unidad"** — en el historial (Compras y la pestaña de Existencias), en el vale impreso, en los últimos movimientos del Dashboard y en la ficha del producto. Cubre también todas las filas históricas, porque corrige lo que se muestra y no lo que se guardó. El filtro "Venta fraccionada" sigue trayendo también las ventas por unidad (por dentro son el mismo registro, y el pie de la tabla lo aclara). **El arreglo de fondo quedó anotado en Pendientes**: un tipo `venta_unidad` propio en el enum + backfill de las filas de enteros — se hará junto con otra migración.'],
              ['**27/8/2026**', '**EL HISTORIAL DE MOVIMIENTOS LLEGÓ A ALMACÉN, con más datos por fila** (pedido del dueño: "quiero fecha, hora, qué usuario hizo la operación y el dato que importe"). Tres piezas: **(1)** Existencias ganó la pestaña **Movimientos** (primero fue una sección del menú; el dueño la pidió como pestaña adentro de Existencias el mismo día) — la misma pantalla que Compras › Historial, ahora al lado de la foto del stock porque es su película, sin cruzar de módulo ni gastar una entrada del menú. **(2)** La tabla ganó columnas: **Fecha y hora** (ya estaba la hora, ahora el encabezado lo dice), **Detalle** — el documento o motivo que generó el movimiento ("Recepción remito 0001-7 · BAVOSI", "Venta 0001-00000042 · Consumidor Final", el porqué del ajuste), con el cambio de estado cuando lo hubo (cuarentena, viaje, resolución) — **Valor** (solo las bajas por pérdida: van valuadas al costo congelado del día, el número que suma el reporte y no cambia después) y el botón **Ver** que abre la transferencia o la incidencia de la que nació la fila. La presentación se plegó abajo del producto. **(3)** Cada fila de **Existencias** ganó el botón **"Movs."**: abre la pestaña Movimientos con ese producto y esa sucursal ya filtrados — "¿quién tocó esto y cuándo?" en un clic. Yapa del camino: el estado **en_transito** se mostraba con el nombre crudo del enum (faltaba en el mapa de etiquetas) — ahora dice **"En viaje"**. Verificado en pantalla: las dos pestañas, el salto filtrado desde una fila, y las filas completas ("26/8 08:28 p. m. · Compra · +25 kg · Recepción remito 0001-7 · BAVOSI S.A. · Lucas").'],
              ['**27/8/2026**', '**NACIÓ EL RELEVO DE CAJA: "la cajera se ausenta, cobra el repositor" — sin cambiar de sesión** (migración 0088, pedido del dueño con las tres decisiones confirmadas: PIN obligatorio, habilitación por usuario, y el relevo firma TODO lo de la registradora). El caso real: Karen entra con su usuario y vende; se ausenta y pasa Juan — antes todo seguía firmado por Karen y "quién cerró mal esta venta" no tenía respuesta. **Cómo funciona**: en Gerencia › Usuarios y roles cada usuario puede llevar la marca **"Puede relevar en caja" + su PIN** (4-6 dígitos, guardado con el mismo scrypt que las contraseñas, nunca vuelve al navegador); en el POS, el botón **"Relevo"** (junto al turno) abre "¿Quién está en la caja?": Juan se elige y tipea SU PIN — cinco fallos seguidos lo bloquean 5 minutos. Desde ahí **todo lo que escribe la registradora queda firmado por Juan**: ventas que arma y cobra (vendedor y `cobradoPor`), el movimiento de stock, movimientos manuales de caja, controles de conteo, cobranzas y pagos a proveedor desde el cajón. Mientras el relevo está puesto, un **banner ámbar** lo canta en la registradora y en la lista ("COBRANDO: JUAN — sesión de Karen") con el botón "Volvió Karen" a un clic (sin PIN: la sesión ES de ella) — el banner es la mitad de la solución, porque el modo real de falla es olvidarse de volver. **Cerrar el turno resetea el relevo.** Cada ida y vuelta queda en la tabla de **auditoría** (entidad caja: "Quién está en la caja: Karen → Juan", con turno y hora). Lo que el relevo NO hace, dicho con honestidad: no cambia permisos (la sesión sigue mandando — Juan opera el POS de Karen aunque no tenga la sección), y **la plata sigue siendo del turno**, no de la persona: el sistema dice qué ventas hizo cada uno, no de quién es el faltante del cajón. Técnica: el campo viaja como `operadorId` y NO `usuarioId` (el AutorInterceptor pisa ese nombre con el de la sesión — misma trampa que `paraUsuarioId` de delegar, y en la prueba mordió: el PIN se verificaba contra la cajera), el servidor revalida la marca en CADA escritura (`resolverOperador`), y el PIN incorrecto es **403, no 401** — para el httpClient un 401 es sesión vencida y echaba a Karen al login. El detalle de la venta ganó la fila "Cobrada por" cuando difiere del vendedor. Verificado de punta a punta en pantalla con usuarios descartables: marca+PIN desde Gerencia, PIN malo rechazado sin tirar la sesión, venta cobrada por el relevo firmada entera por él (venta, cobradoPor y movimiento de stock), vuelta de la titular y las dos filas de auditoría. Datos de prueba borrados.'],
              ['**27/8/2026**', '**EL REDONDEO DEL COBRO llegó al POS** (pedido del dueño — venía del sistema viejo: "$39.893 se cobra $39.900", para no pelear el vuelto chico en efectivo). En el modal de Cobrar, pegados al total, aparecen chips con los **números redondos alcanzables** (el próximo $100, $500 y $1.000 hacia arriba, mientras el salto no pase el **tope de $100**; deduplicados, y ninguno si el total ya es redondo): un clic y el total pasa al número redondo, el efectivo del pago único se acomoda solo, y el aviso dice cuánto se sumó con su "quitar". **Solo al contado y solo hacia arriba** — en cta. cte. el comprobante va por su total exacto (la pantalla lo apaga y la API lo rechaza), y cobrar de menos es un descuento, que tiene su circuito. **Cómo viaja por dentro**: el importe va con la confirmación y el servidor lo materializa como el **extra "Redondeo" con IVA 0** — por eso el total sube EXACTO lo pedido (con 21% el paso mínimo es de 1,21 centavos y el número redondo podía ser inalcanzable) y por eso viaja gratis por todo lo que ya existía: el ticket impreso lo lista como renglón, ARCA lo recibe como base al 0% (alícuota id 3), la NC total lo devuelve y el margen lo cuenta como ganancia pura. Es **idempotente**: el reintento de un cobro que falló a mitad (turno cerrado, conexión) limpia el redondeo del intento anterior antes de aplicar el nuevo — no se duplica jamás. La pantalla "Venta registrada" ganó además las filas de extras, que faltaban: el total de abajo ahora se explica con las filas de arriba. Verificado en pantalla con venta real en local: $2.358 → chip "Redondear a $2.400 (+$42)" → ticket con renglón Redondeo $42, neto $1.990,76 + IVA $409,24 = **$2.400,00 exacto**, pago efectivo $2.400, y en la base el extra con IVA 0. Venta de prueba borrada y stock restaurado.'],
              ['**27/8/2026**', '**EL LOGIN DEL SUPERADMIN QUEDÓ SIN FRICCIÓN: se fue la opción "No especificar"** (pedido del dueño). Ahora **dejar el campo Sucursal vacío ya ES no especificar**: el superadmin elige su usuario, tipea la clave y Continuar — entra parado en la central sin tocar nada más. La pantalla no puede esconder el campo "solo para él" porque el login público sigue **sin saber quién es superadmin** (decisión de seguridad del 26/8: publicarlo sería regalar a quién atacarle la clave), así que lo predeterminado es "vacío que el servidor juzga": a cualquier otro rol que lo deje vacío el servidor lo rechaza con "Elegí la sucursal con la que vas a operar" y el formulario conserva usuario y clave. La confirmación con el campo vacío avisa a los dos públicos: "Así entra solo el superadmin (parado en la central); si no lo sos, volvé y elegí la sucursal". En equipos registrados nada cambia: la terminal sigue mandando para todos (0081). Sin cambios de API. Verificado en pantalla con usuarios descartables: el desplegable ya no ofrece la opción, superadmin con campo vacío entró parado en Distribuidora (sucursal 1 en la sesión), y el cajero con campo vacío vio el aviso y recibió el rechazo del servidor.'],
              ['**27/8/2026**', '**LA IMPORTACIÓN DE PROVEEDORES SE MUDÓ AL MÓDULO PROVEEDORES** (pedido del dueño). El botón "Importar proveedores", la columna **Migración** ("35 de 64" + tilde manual "Completa") y el filtro "Solo migración pendiente" ahora viven en **Proveedores › Proveedores** (el padrón), donde siempre estuvo el ABM de la ficha — la importación alimenta el padrón, no los costos. En **Compras › Costos y percepciones** queda solo lo operativo (operaciones, costos con la regla masiva, percepciones y cuenta), con su descripción apuntando al módulo. Detalle técnico: el módulo Proveedores no carga el catálogo de productos, así que el conteo "cuántos ya tienen formato de compra" ahora lo trae la API con el padrón (`GET /proveedores` suma `productosCargados` en una consulta agrupada) en vez de contarse en memoria. Los endpoints no cambiaron y ya admitían la llave `proveedores.padron`. Verificado en pantalla: modal completo en su casa nueva (CSV real de 169 filas → "0 creados y 167 completados", idempotente), tilde con click real en las dos direcciones con el contador de pendientes acompañando (169→168→169), y Compras sin rastro de la parte de migración.'],
              ['**26/8/2026**', '**LLEGÓ LA MERCADERÍA, LA FACTURA VIENE EN CAMINO: el circuito del remito pendiente de facturar.** El caso real (Costa Oliva): el proveedor entrega y la factura tarda días — la mercadería no puede quedar sin vender, pero con stock 0 el POS no la vende, y cargar la factura después la ingresaría DOS veces. El circuito, espejo del "⚠ Sin facturar" de Ventas: **(1)** el día del reparto se carga un **Remito** (Compras › Facturación, tipo que ya existía): ingresa el stock a costo de catálogo, no genera deuda ni es fiscal, y el POS vende desde ese momento. **(2)** Mientras haya alguno, aparece la pestaña **"⚠ Remitos sin facturar (N)"** en Facturación, y el detalle del remito lo canta arriba de todo. **(3)** Cuando llega el papel: **"Llegó la factura"** (en la pestaña o en el detalle) — el mismo asistente de 3 pasos en modo conversión: producto y cantidad CLAVADOS (son lo que entró; una diferencia de entrega va por ajuste o NC), costo/desc/IVA editables como diga el papel, pie completo (bonificación, percepciones), diferencias de costo con actualización de catálogo, compromiso del diferido prellenado y el paso de pago entero. El remito **PASA A SER la factura** (mismo comprobante, rastro en observaciones): nace la deuda y **el stock no se vuelve a mover** — `POST /comprobantes/:id/facturar` recalcula el pie con el MISMO método del alta (se extrajo `armarPie` + compromisos + pago para que las dos puertas no diverjan jamás). **(4) El candado**: al cargar una factura suelta de un proveedor con remitos pendientes, el paso 1 avisa con el remito a un clic — y la API rechaza facturar lo que no es un remito confirmado. Sin migración de base. Verificado entero en local: remito → venta del stock posible → conversión con costo corregido ($5.000→$5.500, catálogo actualizado) → mismo id como factura, stock intacto, deuda y compromiso (26/8+10d) creados, pestaña ⚠ apagada; y el alta normal con pago contado sigue igual tras el refactor.'],
              ['**26/8/2026**', '**ARRANCÓ LA MIGRACIÓN MASIVA: importador de proveedores + tablero de avance + el arreglo multi-proveedor del importador de catálogos.** El plan del dueño: el administrativo de Distribuidora carga primero el padrón de proveedores y después, proveedor por proveedor y de a poco, los catálogos (productos + F. compra + F. venta, el importador que ya existía). Tres piezas nuevas. **(1) "Importar proveedores"** (Compras › Costos y percepciones): un solo CSV —el export "proveedores-ficha" del sistema viejo, reconocido por sus columnas— con vista previa obligatoria. Importa LA FICHA (nombre, CUIT, contacto, Emite→condición de compra con Remito=Sin factura 100, cómo cobra, modo de cuenta, cuentas bancarias) y **los saldos NO, a propósito**: el saldo del CRM nace de los comprobantes, y los saldos iniciales son un paso propio el día del corte (quedó en Pendientes). Es **idempotente**: empareja por CUIT y por nombre normalizado — lo que ya existe se COMPLETA (solo campos vacíos, nunca pisa lo cargado a mano), lo parecido queda **DUDOSO** con selector para emparejar mirando ("NUEVO COSMOS S.A" del archivo ES el "Nuevo Cosmo - Lucfel" cargado a mano), las filas GASTOS* entran clasificadas como proveedores de gastos y las de prueba vienen destildadas. Los CUITs sucios (el "1" de 3 ARROYOS) se descartan con aviso; los **13 mixtos** quedan marcados para cargarles el "Sin factura %" a mano (pendiente que ya existía). **(2) El tablero de avance** en esa misma lista: columna **Migración** con "35 de 64" (formatos de compra cargados contra los productos que el proveedor tiene allá — la columna "Productos asociados" del export, guardada como referencia en la 0087), el filtro "Solo migración pendiente (N)" y el **tilde manual "Completa"** — manual a propósito: el número viejo puede incluir discontinuados que nunca van a migrar; el que sabe si terminó es el que carga. **(3) El candado multi-proveedor** en el importador de catálogos, que era el agujero que el dueño olió: un producto que YA EXISTE (lo trajo otro proveedor primero) se **salteaba entero** y el catálogo del segundo proveedor lo dejaba sin su formato de compra, sin aviso. Ahora se **VINCULA**: se le agrega el formato de compra de este proveedor sin tocar el precio (el primero lo sigue fijando; el nuevo entra con `usarParaPrecio: false`, salvo que el producto no tuviera ninguno), con su tabla propia en el resultado. Verificado con el CSV REAL de producción en la base local: 169 filas → vista previa correcta (TOTALES afuera, CUIT inválido descartado, 13 mixtos contados, 2 de prueba destildadas) → 167 completados sin duplicar ninguno en segunda pasada; y la vinculación por API: producto compartido entre dos proveedores quedó con los dos formatos, el primero fijando el precio.'],
              ['**26/8/2026**', '**EL SUPERADMIN ENTRA SIN ELEGIR SUCURSAL** (pedido del dueño: loguearse desde cualquier máquina sin declarar de qué local es). En el desplegable del login hay una opción nueva al final — **"No especificar — solo el superadmin"** — que viaja sin sucursal y deja que el servidor decida: si el que entra es superadmin, la sesión nace **parada en la central** (la primera sucursal) y se cambia cuando haga falta con el selector del encabezado que ya tenía; **a cualquier otro rol lo rechaza** con su mensaje de siempre, porque para el resto la sucursal es su contexto de trabajo y de arqueo. Dos decisiones de diseño que importan: **(1)** quién es superadmin se decide EN EL SERVIDOR y con la contraseña ya verificada — el login público sigue sin decir quién es quién (publicarlo sería regalar a quién conviene atacarle la clave, el mismo candado de siempre); **(2)** en un **equipo registrado** la terminal sigue mandando para todos, superadmin incluido (0081): ahí no hay pregunta para nadie. Verificado por API real (superadmin sin sucursal → entra en Distribuidora; con sucursal elegida → se respeta; cajero sin sucursal → rechazado) y en pantalla con la confirmación mostrando "Entrás parado en la central y la cambiás desde el encabezado".'],
              ['**26/8/2026**', '**EL CLIENTE DEL POS SE ELIGE EN UN MODAL CON BUSCADOR** (pedido del dueño). El `<select>` nativo con el padrón entero adentro no dejaba buscar por fantasía ni por documento, y con la lista creciendo se volvía inmanejable. Ahora el recuadro CLIENTE es un botón que abre **"Elegir cliente"**: buscador arriba (filtra por nombre, fantasía y documento, con foco automático), lista clickeable abajo — **Consumidor Final SIEMPRE primero** y resaltado si es el actual, y cada cliente con su documento, condición de IVA, cta. cte. y descuento a la vista, que es lo que se necesita ver antes de elegirlo. **Enter elige el primero de la lista** (para el que tipea tres letras y confirma), click elige cualquiera, y **Esc cierra solo el modal** sin salir de la registradora. El default no cambió: sin elegir nada, la venta es de Consumidor Final. Detalle técnico que importa: mientras el modal está abierto, **las teclas del POS quedan bloqueadas** (F2 habría cobrado la venta con el modal adelante) — listener en fase de captura con `stopImmediatePropagation`, que gana siempre. Verificado en pantalla con la caja abierta: filtro "mar" → quedó solo Maria, click la eligió y el botón la muestra; "eje" + Enter eligió a ejem; F2 con el modal abierto no cobró; Esc cerró el modal y la registradora siguió. **Y la previsión del padrón que crece: la lista está PAGINADA** (el mismo paginador de las tablas del sistema, 20 por página con el tamaño recordado por puesto) — la barra recién aparece cuando la lista no entra en una página, así que hoy con pocos clientes no se ve, y **cambiar la búsqueda vuelve solo a la página 1**. Verificado con 33 clientes: "1–20 de 33", página 2 mostró los 13 restantes, y buscar desde la página 2 no dejó la lista colgada en una página vacía.'],
              ['**26/8/2026**', '**"QUITAR EL FONDO" LLEGÓ A LAS TANDAS DE FOTOS — y de paso se arregló el caso de las fotos chicas.** Contexto de la pregunta del dueño: las tres subidas (de a una, lote y ráfaga) ya comprimían igual — molde 800×800 + WebP calidad 0,85, la foto de celular de 4 MB queda en 30–80 KB — pero sacar el fondo existía SOLO en la de a una (con su vista previa). Ahora **lote y ráfaga tienen el tilde "Quitar el fondo"**, opcional y **apagado de fábrica** a propósito: en tanda no hay ojo humano por foto y el flood-fill puede comerse un producto claro que toque el borde — el tilde lo dice, y la que salga mal se rehace con el botón Subir de ese producto, que tiene el "Restaurar fondo". **El hallazgo de la prueba**: una foto MÁS CHICA que el molde (menos de 800px, p. ej. 600×600 de catálogo) volvía INTACTA del quitado de fondo — `moldear` la centra con margen transparente, y la estimación del color de fondo muestreaba el contorno del canvas (todo transparente) y se rendía. Afectaba también al botón de a una desde siempre y nadie lo había pisado porque las fotos de celular son grandes. El arreglo: si el contorno del canvas no dice nada, se muestrea **el contorno del rectángulo real de la foto**, que es donde vive su fondo. Verificado en pantalla con el peor caso a propósito (imagen de 400×400, fondo gris claro, producto oscuro): subida por el lote con el tilde puesto → lo guardado quedó **WebP de 4,5 KB con el fondo 100% transparente** y el producto intacto.'],
              ['**26/8/2026**', '**SISTEMA › RESPALDOS DEJÓ DE DECIR "PRONTO": la copia externa de la base, en un botón.** La decisión con el dueño fue la versión CHICA y honesta: los backups automáticos ya corren en Dokploy y la restauración se hace allá — la pantalla no los duplica ni los ve (los archivos viven fuera del contenedor). Lo que agrega es lo que el VPS solo NO cubre: **(1) la copia de afuera** — "Descargar respaldo (.sql)" genera el volcado completo de la base y lo baja a la máquina (si el servidor se cae con sus backups adentro, la copia externa es la que salva; llevarla a un pendrive o Drive cada tanto), y **(2) el rastro** — cada descarga queda en la tabla `auditoria` (quién, cuándo, tamaño) y la pantalla lista las últimas, porque "hace tres meses que nadie baja una copia" tiene que estar a la vista. **El volcado se genera SIN pg_dump a propósito**: la imagen del contenedor es `node` pelado y no trae el binario (y un pg_dump de versión distinta a la del servidor se niega a correr) — en su lugar la API lee cada tabla con los valores en su representación de TEXTO de Postgres (sin parsear tipos: timestamps, jsonb, arrays y bytea viajan tal cual) y emite INSERTs, con TRUNCATE inicial, `session_replication_role = replica` (por eso la restauración pide superusuario) y las secuencias puestas al día al final. **Las instrucciones de restauración van en el encabezado del propio archivo**, que es donde se buscan en la urgencia: base nueva → `node dist/db/migrate.js` → `psql -f archivo.sql`. **La restauración se probó de verdad**, no se supuso: el volcado se cargó en una base limpia recién migrada y los conteos de productos, ventas, comprobantes, stock y renglones dieron idénticos, con las secuencias andando (un insert posterior tomó el id siguiente sin chocar). Dos yapas técnicas: `httpClient.descargar()` (fetch con token + nombre del Content-Disposition, sin caché — un respaldo viejo servido como nuevo es peor que ninguno) y `exposedHeaders: [\'Content-Disposition\']` en el CORS de la API, sin lo cual el navegador no puede leer el nombre del archivo. **Queda una tarea del lado del servidor**: verificar en Dokploy que el backup automático de Postgres esté configurado y probado — está en Pendientes.'],
              ['**26/8/2026**', '**NACIÓ LA AUDITORÍA DE CAMBIOS — la pestaña del proveedor dejó de decir "Próximamente"** (migración 0086, decisión del dueño: "arrancá por las condiciones comerciales"). El problema que resuelve: las condiciones comerciales se pisaban EN EL LUGAR y sin firma — alguien tocaba el "Sin factura %" de un formato y el margen de noventa productos cambiaba en silencio; los documentos (facturas, pagos, anulaciones) ya quedaban firmados en sus tablas, pero los cambios A MANO no tenían rastro. Ahora hay una tabla **`auditoria` GENERAL a propósito** (entidad + id: es el MISMO mecanismo que va a alimentar Gerencia › Auditoría cuando se construya esa vista, sin migrar nada) donde cada fila es **UN campo que cambió**: fecha y hora, quién, dónde (ámbito + detalle), y **antes → después en palabras legibles** ("Flete %: 0% → 10%", "Plazo de pago: Sin plazo → 15 día(s)") — nunca "se guardó el formulario", y **solo lo que cambió de verdad** (guardar sin tocar no ensucia el registro). La escriben los tres caminos de las condiciones comerciales: el **formato de compra** (bulto, costo, descuentos, flete, sin factura, quién fija el precio — con el producto como detalle, y el alta/baja del formato como fila resumen), las **percepciones** (el full-replace se traduce a agregada/quitada/modificada por nombre) y la **ficha del proveedor** (nombre, CUIT, condición de compra, medio habitual, plazo, modo de cuenta, sin factura default). Se graba **dentro de la misma transacción** que el cambio: un registro que puede quedar sin su cambio miente. La lee la pestaña **Auditoría** del modal del proveedor (Compras › Costos y percepciones); el endpoint (`GET /auditoria`) valida entidad y pide las llaves de quienes administran proveedores. Ojo del deploy: la migración a mano exige **registrarse en `drizzle/meta/_journal.json`** o el migrador la ignora en silencio — "Migraciones aplicadas" sin crear la tabla. Verificado de punta a punta con datos descartables: 6 cambios reales (ficha, percepción nueva, alta de formato y dos ediciones) quedaron cada uno con su fila firmada, y la pestaña los mostró.'],
              ['**26/8/2026**', '**EL COMPROBANTE DE COMPRA YA NO NACE CON TIPO Y PROVEEDOR PUESTOS.** El paso 1 del alta venía precargado con "Factura" y el primer proveedor del padrón — y NUEVO COSMOS no es el proveedor de nada por estar primero en la lista: un Continuar distraído metía la compra entera en el proveedor equivocado, que es exactamente lo que el dueño señaló. Ahora **Tipo y Proveedor arrancan en "Elegí…"** (opción deshabilitada: se ve mientras no se eligió, no se puede volver a ella) y el Continuar corta con su aviso si falta cualquiera de los dos. Los prellenados que SÍ son un dato se conservan: la factura que viene de la **bandeja** trae tipo y proveedor del QR, y el alta desde la **ficha del proveedor** ya sabe de quién es. De paso, la sucursal de recepción recién aparece cuando se elige el tipo (antes no se sabe si el comprobante recibe mercadería). Verificado en pantalla: vacío → "Elegí el tipo", con tipo → "Elegí el proveedor", con ambos → pasa a Ítems.'],
              ['**26/8/2026**', '**HABILITAR CUENTA CORRIENTE ES DEL SUPERADMIN: se separó del alta del cliente.** Pedido del dueño: el admin crea y edita el cliente con todos sus datos, pero **habilitar la cuenta corriente y asignarle el monto** (límite y plazo) lo decide solo él — y todo sigue en el mismo formulario de Ventas › Clientes. Nació la acción **`cta_cte`** ("Clientes: habilitar cuenta corriente y fijar límite"), que **ningún rol trae de fábrica**: solo pasa el `*` del superadmin, y si un día quiere delegarla se tilda en Gerencia › Usuarios y roles sin tocar código (mismo patrón que `nota_credito`). El candado es **a nivel campo, no de pantalla**: sin la llave, la sección Cuenta corriente del formulario muestra el estado como info ("Habilitada — límite $50.000 · plazo 30 días" o "No habilitada") con la aclaración de quién la maneja, los tres campos no viajan en el guardado, **y la API los preserva tal como están** — el detalle técnico que importa: el PATCH traducía campo ausente a false/0, así que sin la preservación explícita un admin guardando el teléfono APAGABA la cuenta corriente de paso. Un intento de cambiarlos por atrás (POST a mano) se **rechaza con 403 y mensaje con nombre**, no se ignora en silencio: un candado mudo enseña que el botón anda. Verificado con 7 casos por API real (admin crea con crédito → 403; admin edita teléfono → crédito intacto; admin manda los mismos valores sin cambiar → pasa; superadmin habilita → pasa) y las dos vistas en pantalla con usuarios de cada rol. **Y el mismo día, el atajo**: en el detalle del cliente, pestaña Cuenta corriente, quien tiene la llave ve un mini-formulario **"Habilitar ahora"** (límite + plazo, prellenados con los defaults de la config) y habilita ahí mismo, sin pasar por la edición completa — el modal queda abierto y la pestaña pasa sola a mostrar la cuenta. Endpoint propio y acotado (`PATCH /clientes/:id/credito`, llave `cta_cte` a nivel endpoint, mismo patrón que el cartel de góndola), que además rechaza habilitarle crédito al Consumidor Final — fiarle a "nadie en particular" no tiene a quién cobrarse después.'],
              ['**26/8/2026**', '**EL EFECTIVO SE CUENTA POR BILLETE: nació el contador del arqueo.** Al cerrar caja (y en el control intermedio, que es el mismo gesto) el campo "Efectivo contado" pedía UN número — o sea que la cuenta se hacía afuera, en una calculadora o en un papel, y lo que entraba al arqueo era el resultado de esa cuenta, con sus errores de suma incluidos. Ahora abajo del campo hay un botón **"🧮 Contar billetes"** que abre un modal chico con **las 17 denominaciones vigentes, de $0,05 al billete de $20.000**, de mayor a menor como se apila el cajón: se tipea la CANTIDAD de cada una, el subtotal del renglón y el total se calculan solos, y **"Usar este total"** lo deja puesto en "Efectivo contado" — la diferencia contra lo esperado se recalcula al instante, como si se hubiera tipeado a mano. Detalles que valen: **Enter baja al renglón siguiente** (se cuenta de corrido, sin mouse; Tab también funciona porque el orden del foco ES el orden de las denominaciones), el conteo **se conserva mientras el modal del arqueo siga abierto** (se puede volver a entrar a corregir una cantidad sin re-tipear todo), y la cuenta interna va **en centavos** — 3 monedas de $0,05 son exactamente $0,15 y no un flotante con basura, que en un arqueo donde la diferencia se mide contra $0,009 no es un detalle. El conteo NO se guarda en la base: es una ayuda para llenar bien el campo de siempre, así que **no hubo cambio de API ni migración**. Verificado en pantalla: 2×$20.000 + 3×$10.000 + 5×$2.000 + 3×$0,05 = $80.000,15 exactos, el total pasó al campo, la diferencia dio $53.810,15 contra los $26.190,00 esperados, y al reabrir el contador las cantidades seguían. (El salto con Enter quedó verificado por código y foco nativo: el panel de prueba de esta sesión inyectaba las teclas con `key` vacío, cosa que un teclado real no hace.)'],
              ['**25/8/2026**', '**FOTOS DE LA TIENDA EN LOTE: las dos piezas del plan, construidas.** De a una eran ~5 acciones por producto; ahora en **Web › Productos** hay tres cosas nuevas. **(1) "Subir fotos en lote"**: se arrastran decenas de archivos y cada uno se empareja con su producto **por el nombre del archivo** — código de barras por token exacto (infalible, y es como vienen los catálogos de los proveedores), código propio, o nombre normalizado (igualdad = emparejado; parecido = DUDOSO con el candidato precargado, porque "alfajor chocolate" pega con tres alfajores y elegir por el sistema sería adivinar). Pantalla de revisión con chips (EMPAREJADO / CONFIRMAR / SIN DESTINO), selector por fila para los sueltos, aviso de "ya tiene foto: la reemplaza" y de dos archivos al mismo producto, y subida secuencial con progreso. **(2) "Modo ráfaga"**: recorre SOLO los sin foto mostrando cuál toca — se le suelta la foto, se guarda sola y pasa al siguiente; para las fotos del celular (IMG_2043.jpg) donde no hay nombre que emparejar. **(3)** el filtro **"Ver solo sin foto (N)"**, que es el contador del hueco y la lista de trabajo. Todo usa **el mismo molde de siempre** (800×800 → WebP → POST por producto, que valida igual): el único cambio de API fue sumar los códigos a `/web/productos` (el catálogo público no los trae, y no tiene por qué). Verificado en el navegador: 3 archivos soltados → el del código de barras y el del nombre exacto EMPAREJADOS, el basura SIN DESTINO, subida real a la base como WebP; y en ráfaga, saltear + soltar + avance automático. La elección de archivos del sistema no se puede automatizar desde el panel: el "soltar" de la prueba fue un DataTransfer con PNGs generados reales — todo lo demás (emparejado, molde, POST, base) corrió en serio.'],
              ['**25/8/2026**', '**EL BULTO DE LA FICHA SE COMPLETA SOLO AL GUARDAR EL FORMATO DE COMPRA.** Lo destapó producción: un producto con "Unidades por bulto 12" en su Formato de Compra que en el pedido solo se dejaba pedir por unidad. Son **dos campos distintos**: el del formato es del proveedor (cuántas trae la caja que factura) y el pedido por caja lee el de la FICHA (`productos.unidadesPorBulto`, el del DUN) — en dev los emparejó el script `db:bulto`, en producción nunca se corrió. Dos arreglos: **(1) para adelante**, al guardar formatos de compra la API completa la ficha sola, con las reglas del script (solo rellena lo VACÍO — nunca pisa lo cargado a mano—, solo enteros, y solo si todos los formatos declaran el mismo bulto; en desacuerdo no inventa: lo deja para el ojo humano). Verificado por API real: ficha en 1 + formato x6 → guardar → ficha en 6, formato intacto. **(2) para lo ya cargado en producción**, correr el script UNA vez desde la terminal del contenedor de la API en Dokploy: `node dist/db/cargar-bulto.js` (vista previa, no escribe) y después `node dist/db/cargar-bulto.js --aplicar` — es el mismo `npm run db:bulto` pero compilado, porque el contenedor no tiene ts-node.'],
              ['**25/8/2026**', '**LA TANDA DE CANDADOS: el bloque rojo de Pendientes, barrido entero.** El dueño pidió arrancar por ahí (los usuarios de prueba siguen con 1234 a pedido suyo: los reales se crean con clave propia). Al revisar, la mitad de la lista ya estaba resuelta por la tanda del 22/8 y quedó verificada en vivo (helmet con sus cabeceras, mime contra bytes en Gastos y Web, archivos de facturas con 401 sin sesión, el turno de la cobranza resuelto por el servidor). Lo que sí se construyó hoy: **(1) una sola caja abierta por sucursal** — índice único parcial en la base (0085) + traducción del 23505 al mensaje amable; probado con dos aperturas EN PARALELO: entra una. **(2) El autor de toda operación de inventario sale de la sesión** — diez puertas aceptaban `usuarioId` del body; barrida completa de DTOs y handlers, probado con un spoof: la traza quedó con el usuario real. **(3) La cobranza toma y anula su turno CON candado** — FOR UPDATE + revalidación dentro de la transacción, espejo de lo que la venta ganó a la mañana. **(4) Un borrador no puede recibir mercadería** — antes movía stock sin deuda y sin camino de confirmación. **(5) La yapa descubierta revisando**: el cierre de la API había roto las miniaturas de la bandeja de facturas (un `<img src>` no manda el token) — nació `httpClient.urlProtegida` (fetch con Bearer → URL `blob:` cacheada) y las miniaturas y los «Ver el papel» volvieron a andar. Queda del bloque rojo solo lo que es del servidor de producción: rotar la clave de la base expuesta (se hace en Dokploy).'],
              ['**25/8/2026**', '**EL CARTEL SE DISEÑA ARRASTRANDO: nació el editor de la etiqueta** (Ventas › Carteles › &ldquo;Diseñar el cartel&rdquo;). Los carteles seguían saliendo cortados: el diseño proporcional adivinaba el layout a partir del alto del rollo y contra la plantilla física nunca terminaba de calzar. El dueño pidió lo correcto — *&ldquo;un recuadro con las medidas exactas, y yo muevo cada elemento a su lugar&rdquo;* — y eso es exactamente lo que se construyó: **el lienzo es la etiqueta a escala** (64×32 real, cuadrícula de 5 mm) y cada elemento (recuadro, marca, nombre, precio minorista, mayorista) **se agarra y se arrastra**; al lado, campos numéricos para afinar a 0,5 mm y la letra en mm. **LA CLAVE DEL DISEÑO: las posiciones se guardan EN MILÍMETROS y la impresión usa esos mismos números** (elementos absolutos en mm dentro de la hoja) — no hay traducción entre lo que se acomoda y lo que sale, que era el pecado del sistema anterior de fórmulas. La vista previa del editor se genera **con el mismo código que imprime**, así que es la verdad, no un dibujito. La plantilla es **POR FORMATO** (diseñar el 64×32 no toca un rollo más grande) y vive en la **config de impresión del servidor** — la comparten todas las máquinas, no queda en un navegador; guardarla pide el permiso de Impresión de Sistema. Sin plantilla guardada rige el diseño estándar de siempre, y &ldquo;Volver al diseño estándar&rdquo; la borra. Las reglas de contenido no cambian: línea vacía no se imprime, mayorista solo si el producto tiene ese precio, y el texto largo se achica solo hasta entrar en el ancho que se le dio al elemento. **Detalles técnicos que valen**: la plantilla viaja como JSON en string dentro de la config (el sanitize de configuración trabaja con campos planos; la regla `impresion.plantillaCartel` parsea, poda claves y números con topes, y re-serializa — el portero sigue existiendo), y el footer de ModalShell espera `{texto, onClick}`, no JSX — pasarle botones armados renderiza cuatro botones vacíos que no hacen nada, y se descubre solo apretándolos. **Y LA ETIQUETA DEL FRACCIONADO SE DISEÑA IGUAL** (&ldquo;hacelo igual&rdquo;, el mismo día): botón &ldquo;Diseñar la etiqueta&rdquo; en Almacén › Fraccionamiento › Etiquetas, con SUS elementos — nombre, peso, precio, **código de barras con su ancho y alto propios**, el número del código y el vencimiento (clave `plantillaFraccionado`, saneador compartido por esquema en la API). Ojo al angostar el código de barras en el diseñador: abajo de 0,25 mm por barra la térmica imprime lindo y el lector no lee nunca — el editor lo avisa. La vista previa de Sistema › Impresión también respeta las plantillas guardadas. Son dos componentes primos y NO una abstracción compartida a propósito: los elementos son otros, y un genérico que contemple a los dos se vuelve ilegible para ajustar cualquiera. Y del mismo día: la letra del elemento seleccionado se agranda y achica con **botones A− / A+** al lado del campo (pasos de 0,5 mm, entre 1 y 40) — el campo numérico solo estaba, pero el dueño no lo encontró, y un control que no se encuentra no existe. También del mismo día, tras la primera tanda real: **el diseño estándar del cartel respira** (los precios se reparten en lo que queda debajo del recuadro con aire parejo, en vez de amontonarse pegados a la caja con un hueco muerto al pie — &ldquo;Volver al diseño estándar&rdquo; en el diseñador toma la versión aireada), y **un botón de ayuda al lado de Imprimir** (carteles Y fraccionados) explica cómo apagar el encabezado de Chrome — la línea gris con fecha y título que pisaba la marca en el papel: la agrega el navegador, no el sistema, y se destilda UNA vez en el diálogo (Más opciones → &ldquo;Encabezados y pies de página&rdquo; + Márgenes: Ninguno); la explicación estaba en el chat y no en la pantalla, que es donde se necesita. El botón terminó **en el color de info con un &ldquo;?&rdquo; adelante**: la primera versión era un botón gris más entre los botones y el dueño no lo vio. Y la trampa de estilos que dejó esta pantalla: **`s.card` NO trae padding** — el relleno vive en `s.cardPad` y van juntas (`cx(s.card, s.cardPad)`); una tarjeta con `s.card` sola muestra el texto pegado al borde, que es exactamente como se veía Carteles hasta el 25/8.'],
              ['**25/8/2026**', '**LA COMPRA TAMBIÉN SE CARGA POR UNIDAD SUELTA, cuando la entrega no completa el bulto.** El renglón del comprobante hablaba SOLO en bultos —"llegaron 2 bolsas de 25 kg"— y el dueño señaló el caso que ese idioma no puede decir: **a veces llegan 5 alfajores, no la caja de 12**. La salida a mano era inventar un "bulto de 5", que miente dos veces: le ensucia al proveedor el tamaño del bulto declarado (la tabla de impacto lo detectaría como "el bulto cambió" y ofrecería pisarlo en el formato de compra) y deja un costo por bulto que no es de ningún bulto real. Ahora cada renglón de un producto **ENTERO** tiene un **selector debajo de la cantidad: "bultos" o "u. sueltas"** — el granel NO lo tiene (corrección del dueño el mismo día: ahí el bulto es la bolsa y su tamaño ya se corrige por entrega, "kg sueltos" no agregaba nada y confundía). En modo suelto la cantidad ES el total que entra al stock, el costo es POR UNIDAD, y la columna "Por bulto" muestra un guion — porque **una entrega suelta no dice nada del tamaño de la caja del proveedor**, así que ni se compara ni se toca. **El costo se convierte solo al cambiar de modo** ($49.815,72 el bulto de 12 ↔ $4.151,31 la unidad) y la ida y vuelta devuelve el número exacto: el $/u real no se mueve, que es el que alimenta catálogo, impacto y góndola. Por adentro no cambió nada: el modo suelto es matemáticamente "bultos de tamaño 1" y la API recibe lo de siempre (cantidad total + costo unitario), así que **sin API nueva ni migración**. De paso los encabezados dejaron de decir "Bultos" dos veces (quedaron **Cantidad · Por bulto · Costo**), que era la confusión que originó el pedido. Y la segunda vuelta del mismo día: aun así "1 · bultos · 12" se leía como tres números sueltos ("¿estoy cargando una unidad más un bulto de doce?"), así que **debajo de la cantidad va la cuenta ya hecha, en negrita: "= 36 u."** — lo que entra al stock, misma regla en los dos modos. Se descartó poner el tamaño dentro de la opción del selector ("bultos ×12", el recurso del pedido): en el ancho real de esa columna se trunca a "bultos ×1", que es peor que la ambigüedad original. Y la tercera vuelta cerró el diseño: **el tamaño del bulto dejó de ser editable en el renglón** — es info que sale del Formato de compra del producto y se cambia ahí (enteros y granel por igual). Un casillero editable era otro número tipeable en la misma fila, y ese número define el $/u del catálogo: la factura ya no lo puede pisar de paso. Probado en pantalla con el aceite de NUEVO COSMOS: 5 sueltas = $20.756,55, sin disparar "Impacto en precios", y la vuelta a bultos restaura el costo exacto.'],
              ['**25/8/2026**', '**"NUEVO" ES DEL CATÁLOGO, NO DEL LOCAL — y el otro chip pasó a llamarse REINGRESO.** El dueño corrigió la definición mirando la pantalla real: los chips de las novedades del pedido marcaban NUEVO a todo lo que **ese local** nunca había tenido, así que a una sucursal sin historia le gritaba NUEVO el catálogo entero — que es exactamente lo que se veía en Express 1. La definición corregida: **NUEVO es el producto que SE CREÓ hace poco y entró a stock** (novedad del negocio, algo que el cajero no puede buscar porque no sabe que existe); todo lo que ya existía y volvió a entrar al depósito es **REINGRESO** (chip azul), lo haya tenido ese local o no. Para eso hizo falta un dato que no existía: **cuándo nació cada producto** (`productos.creadoEn`, migración 0084) — desde ahora lo pone el alta sola, y **los 228 existentes quedaron con fecha vieja a propósito** (1/1/2026): sus altas reales no están registradas y adivinar marcaría NUEVO de más, que es la forma más rápida de que el chip se deje de mirar. **La trampa que se comió la primera corrida y vale conocer**: el backfill original era al 1/8 —el corte histórico de las novedades— y esa fecha queda ADENTRO de la ventana de 30 días del chip, así que todo el catálogo volvía a contar como recién creado; la fecha del backfill tiene que quedar fuera de cualquier ventana, y por eso es 1/1 y no 1/8. Las duraciones no cambiaron: NUEVO dura 30 días desde el alta (y se apaga antes si el local ya lo recibió); REINGRESO, hasta el próximo pedido del local. Y de la misma pasada, **el orden**: lo último que entró va arriba, cronológico puro sin agrupar por chip — antes los NUEVO iban todos primero y un reingreso de ayer quedaba enterrado abajo de nuevos de hace tres semanas; de paso apareció que el filtro "Ver solo novedades" cortaba en 40 ANTES de ordenar (con más de 40 sobrevivían las primeras del catálogo, no las últimas cargadas). Probado con el caso que antes fallaba: producto viejo que el local nunca tuvo → REINGRESO, y un solo NUEVO en la lista (el único creado en la ventana).'],
              ['**25/8/2026**', '**EL FLETE VOLVIÓ A LA CADENA DEL SISTEMA ANTERIOR — se cerró la pregunta abierta del 19/8.** El dueño eligió "tal cual Sistel", y esa elección **define los hechos que la hacen correcta**: el flete viene FACTURADO por el transportista (su IVA se recupera como crédito fiscal, igual que el de la mercadería en blanco) y el % cargado es el BRUTO — la fracción de lo que sale del bolsillo. Con eso la cadena vieja `lista × (1−17,36%) × 1,21 × (1+flete%)` deja de ser una rareza y pasa a ser la cuenta exacta. La forma cerrada que quedó en `pricing.ts` (y su espejo del dashboard): **base del precio = (mercadería + flete) × ratio**, donde `ratio = (1−q) + q/1,21` es el MISMO factor por el que ya pasaba la parte sin factura — todo uniforme, una sola regla. Tres consecuencias que valen: (1) el **IVA absorbido es SOLO de la mercadería** — el del flete no se pierde, vuelve como crédito, así que Rentabilidad no lo cuenta como pérdida; (2) el **costo real también baja un poco** con parte sin factura (el flete se valúa en neto), y la identidad `costoNeto = base + absorbido` se mantiene; (3) la identidad de control se volvió MÁS fuerte que la anterior: `base × 1,21 = lo pagado al proveedor + lo pagado al fletero` — la base con IVA reconstruye TODO lo que salió. La regla del 19/8 ("el flete entra entero") suponía flete sin papel y **cobraba de más**: ~$100 por unidad en el aceite de CARLOS OLIVAA, que era exactamente la brecha contra el sistema viejo que originó la pregunta. **Con 0% sin factura nada cambia** — el ratio es 1 y todos los productos facturados quedan idénticos. Verificado por triplicado: 9 comprobaciones numéricas (la base da $2.863,64/u y la góndola $5.197,50, los números de Sistel exactos), la pantalla del formato mostrando la cadena nueva en vivo, y la fila del fletero ahora muestra lo PAGADO ($3.780) con la aclaración de que su IVA vuelve. Las ventas ya congeladas no se tocan: el costo y el absorbido se congelan al vender, así que el cambio rige desde ahora.'],
              ['**22/8/2026**', '**CARTELES DE GÓNDOLA: el precio que el cliente lee en el estante** (migración 0083, **Ventas › Carteles de góndola**). Para que el cliente no tenga que preguntarle el precio al cajero. Es **la misma etiqueta autoadhesiva** que la del fraccionado —misma impresora, mismo rollo— con tres diferencias que salen todas de para quién es: el **nombre va grande** (la del paquete se lee en la mano, el cartel a un metro), **no lleva código de barras** (nadie escanea un cartel) y admite **hasta dos precios**. **LOS DOS PRECIOS SE RESUELVEN SOLOS**: la línea de mayorista aparece si el producto tiene ese precio cargado y no aparece si no — no hay nada que configurar por producto, que era el riesgo de una casilla más. Y las listas se buscan **por modalidad, no por id fijo**: los ids dependen de cómo se sembró cada base. **EL TEXTO ES EDITABLE Y SE GUARDA POR PRODUCTO**, que es el pedido de fondo. El nombre del catálogo sirve para buscar y facturar —"Aceite de oliva intenso lata x500ml"—, no para leerse de lejos; en el cartel va "Aceite Oliva Intenso 500ml". Se escribe una vez y queda: **cuando cambia un precio, rehacer el cartel es apretar Imprimir**. Si el texto viviera solo en la impresión, cada actualización obligaría a reescribir veinte carteles, y el que tiene que reescribir veinte termina imprimiendo el nombre largo. **NULO Y VACÍO SIGNIFICAN COSAS DISTINTAS** y por eso las columnas admiten nulo: **nulo** es "no se personalizó" (el cartel sigue al catálogo, así que si mañana cambia el nombre del producto el cartel lo acompaña) y **el campo vacío** es "esta línea NO se imprime" — que es exactamente lo que permite **el cartel de la marca sola para toda una góndola**. Con un `||` de por medio no se podría: trataría al vacío como ausente y volvería a poner el nombre largo. Hay además **"+ Cartel libre"** para uno sin producto ("ACEITES", "OFERTAS"). **ES UNA LISTA Y NO DE A UNO**, a diferencia del de fraccionados: rehacer una góndola son quince o veinte carteles y de a uno no lo hace nadie. **EL PRECIO NO SE TIPEA** — misma regla que la etiqueta del fraccionado y acá pesa más: el cliente lee el cartel y la caja le cobra lo que dice el sistema, así que un precio a mano es un **precio exhibido que no coincide con el cobrado**. Sí se puede SACAR (cartel sin precio), que es otra cosa que inventarlo. Endpoint **acotado a propósito** (`PATCH /productos/:id/cartel`, `compras.productos` o `ventas.cambios`): los carteles los rehace el que maneja precios, que puede no tener permiso para editar costos y códigos — darle el update completo del producto para que escriba dos líneas sería abrir de más. **Dos errores del camino, los dos por asumir el nombre de una clave**: el catálogo exige `sucursalId` (sin él da 400) y mi `catch` lo tapaba mostrando "nada coincide", que manda a buscar el problema en el producto; y los ítems del catálogo se identifican con **`productoId`, no `id`** —y las listas con `listaId`, no `id`—, así que los carteles salían sin precio y sin reconocer el producto. Los dos aparecieron probando en pantalla, no leyendo. **EL DISEÑO COPIA EL CARTEL DE PAPEL QUE YA SE VENÍA HACIENDO** (el dueño pasó una foto del suyo, hecho en ZebraDesigner): **la MARCA arriba y es lo más grande** —es lo que se busca desde el pasillo—, el **nombre abajo condensado en un solo renglón**, los dos **dentro de un recuadro negro**, y los **precios abajo, a la izquierda, con el rótulo adelante** (MINORISTA: / MAYORISTA:). Todo en mayúsculas por CSS, así se escribe normal en la pantalla y sale en caja alta sin que nadie tenga que acordarse. La primera versión que hice tenía la marca chica arriba y el nombre grande centrado: parecía razonable y **no se parecía en nada al que usan**, que ya está probado en el pasillo. **El texto se achica solo según el largo** en vez de cortarse con puntos suspensivos —un cartel con el nombre cortado no sirve—, y el tope de caracteres quedó **conservador a propósito**: con el primer cálculo, "ACEITE OLIVA INTENSO 500 ML" perdía la L final, y ese recorte no se ve en la pantalla sino cuando ya salieron cincuenta del rollo.'],
              ['**22/8/2026**', '**PEDIR POR CAJA O POR UNIDAD, a elección de la cajera.** Un alfajor viene en caja de 12, y hasta ahora había que hacer la cuenta a mano: querés tres cajas, tipeás 36 y esperás no haberte equivocado. Ahora cada renglón del pedido tiene un selector chiquito al lado de la cantidad —**u.** o **caja ×12**— y la equivalencia siempre a la vista: pidiendo por caja dice "= 36 u.", y pidiendo por unidad dice "= 3 cajas" **cuando el número da un bulto justo** (si no da exacto no dice nada: "2,08 cajas" no le sirve a nadie, ni a la cajera que no puede pedir un pedazo de caja ni al que prepara). **No hubo que agregar ningún dato**: sale de `productos.unidadesPorBulto`, el bulto que identifica el DUN, que ya se carga en la ficha del producto y ya venía en el catálogo del front — así que esto es **puro frontend, sin API nueva ni migración**. **LA DECISIÓN QUE IMPORTA: el renglón sigue guardando UNIDADES, siempre.** La caja es una forma de tipear, no otra unidad de medida. Si el pedido guardara cajas habría que traducir en el autoguardado, en el envío, en la preparación, en el remito y en el stock: cinco lugares donde el mismo número significaría dos cosas distintas, y con que uno se olvide de dividir ya viajan 432 alfajores en vez de 36. Se multiplica una vez, en el casillero, y de la fila para adentro no se entera nadie — **verificado en la base: 3 cajas quedan guardadas como 36**. Dos detalles: al pasar a cajas se **redondea para arriba** al bulto entero (pedir "media caja" de algo que se pide por caja no significa nada, y sin eso el casillero mostraba 0,083 y parecía un error del sistema), y volver a unidades no toca el número. **El selector no aparece** si el producto no tiene bulto cargado —ofrecer "caja ×1" sería un clic al pedo— ni en los productos a granel, donde la unidad de pedido ya es el paquete y "una caja de paquetes de 500 g" no es algo que el sistema sepa. **Y LA OTRA MITAD: el que prepara también la ve.** De nada sirve pedir por caja si el que va a buscar la mercadería lee "36 u." y tiene que dividir de memoria. La equivalencia aparece ahora en la **pantalla de preparación** y —sobre todo— en la **lista impresa**, que es la que se camina por el depósito con la hoja en la mano y sin dónde hacer la cuenta. Se **deriva de las unidades guardadas**, así que no hizo falta que el pedido recuerde cómo se tipeó: si el número da un bulto justo lo dice, y si no, se calla. **EL DATO SE CARGÓ EL 22/8**, y conviene saber de dónde salió. Estaba en **17 de los 64 enteros activos**; se completaron **27 más tomándolos del FORMATO DE COMPRA del proveedor** (`producto_proveedores.cantidad`, el "Caja x12" que ya se carga para comparar costos) — o sea que **no se inventó ni un número**: cada uno salió de un dato que alguien ya había cargado mirando la factura. Los 27 eran unívocos, sin dos proveedores que declararan bultos distintos para el mismo producto. Quedan **20 sin bulto**, que no tienen ninguna fuente: esos hay que ponerlos a mano mirando la caja, y hasta entonces simplemente no muestran el selector (que es el comportamiento correcto, no una falla). **Y quedó una discrepancia para revisar**: *Arandanos Rojos Desh Azuc CUMANA* (id 39) tiene **x10 en la ficha y x11 en el formato de compra**. Es anterior a esta carga y no se tocó — uno de los dos está mal tipeado, y x11 no parece un bulto real. **PARA PRODUCCIÓN hay un script**, `npm run db:bulto`, que **por defecto NO ESCRIBE**: muestra qué cargaría, qué queda ambiguo (dos proveedores con bultos distintos), qué queda sin fuente y qué discrepa, y recién con `-- --aplicar` escribe, en una sola transacción. **No se hizo migración a propósito**: las migraciones corren solas al desplegar, y esto escribe datos que dependen de lo que haya cargado cada base — el resultado no se puede saber de antemano, así que se mira primero. Es **idempotente** y **nunca pisa** un bulto ya cargado: si alguien lo puso a mano mirando la caja, ese dato vale más que el formato de compra. **El 25/8 el renglón se emparejó con el de la factura** (pedido del dueño: &ldquo;haz lo mismo para armar los pedidos&rdquo;): el selector u./caja va **debajo** del casillero y no al costado, la cuenta hecha va **en negrita** (&ldquo;= 6 u.&rdquo;) cuando se pide por caja, y la columna Present. de un ENTERO dejó de ser un desplegable — tenía UNA sola opción (&ldquo;Unidad&rdquo;) y al lado del selector u./caja eran dos controles que decían casi lo mismo; ahora es texto. El granel conserva su select, que ahí sí se elige (kg sueltos o paquetes). Y la segunda vuelta del mismo día lo dejó **calcado a la factura** (&ldquo;hacé exactamente igual esa parte&rdquo;): las columnas quedaron **Producto · Present. · Cantidad · Por bulto · disponibles** — la cantidad se tipea y al lado está el **tamaño del bulto como info** (&ldquo;6 / u./bulto&rdquo;, sale de la ficha y se cambia allá); el selector dice **&ldquo;caja&rdquo; a secas** porque el ×6 ya vive en esa columna. Los disponibles orientan y van después: no se tipean. Tercera vuelta del día: **la columna Present. se fue de la pestaña de enteros** (decía &ldquo;Unidad&rdquo; en todas las filas, y una columna que dice lo mismo siempre no informa nada) — el granel la conserva en SU pestaña, que ahí sí elige (kg sueltos o cuál de sus paquetes). Y el paso 2 ganó **ritmo vertical con un gap parejo** (los márgenes de 4 px sueltos amontonaban aviso, buscador, filtros y tabla — mismo diagnóstico que las tarjetas de Carteles).'],
              ['**22/8/2026**', '**NOVEDADES EN EL PEDIDO: el cajero se entera de lo que llegó al depósito.** Armando el pedido había **solo dos formas de encontrar un producto**: escribir en el buscador, o el filtro "sin stock". O sea que **solo se encontraba lo que ya se sabía que existía** — un producto nuevo era literalmente invisible, porque nadie puede buscar un nombre que nunca escuchó. La mercadería llegaba a la Distribuidora y los locales se enteraban por comentario de pasillo. Ahora, al entrar al paso "Qué se pide", aparece un aviso —*"11 productos que Express 3 nunca tuvo, y 1 repuesto desde tu último pedido"*— con un botón que enciende el filtro **Ver solo novedades**, y cada fila lleva su chip. **DOS CHIPS PORQUE SON DOS PREGUNTAS DISTINTAS, y por eso tienen VENTANAS DISTINTAS**: **NUEVO** (verde) es *"este local nunca lo tuvo"* y responde "¿qué cosas nuevas hay para vender?" — ventana de **30 días**, larga a propósito, porque si entró el martes y el local pidió el miércoles sin darse cuenta, con la ventana corta desaparecía para siempre; **LLEGÓ** (azul) es *"ya lo tuviste y se repuso **desde tu último pedido**"* y responde "¿ya puedo pedir eso que no había?" — ventana corta, porque repetir una reposición todos los días la vuelve ruido. Todo es **relativo al local que pide** (decisión del dueño): "nunca lo tuvo" se mide contra los `movimientos` de ESA sucursal, y sirve que la recepción de una transferencia grabe una fila en cada punta. **TRES FILTROS QUE NO SE VEN Y SIN LOS CUALES ESTO NO SERVIRÍA.** (1) **Se exige stock disponible HOY en el depósito**: "llegó el lunes" no es "hay" —pudo irse el martes—, y una lista que ofrece lo que no está se deja de mirar a la segunda vez. (2) **Solo productos activos**: un discontinuado se sigue vendiendo hasta agotar, pero no es una novedad que haya que empujar. (3) **Un corte histórico (1/8/2026)**: los productos importados del sistema viejo **no tienen compras registradas**, así que sin ese piso el día del estreno aparecían los ~550 como novedad y el cajero cerraba la pestaña para siempre — es la trampa clásica de estas funciones, que nacen gritando y se apagan solas. **El aviso aparece solo si hay algo**, por lo mismo: un cartel que dice "0 novedades" todos los días enseña a no leer los carteles. **Probado** con un escenario armado a mano: los dos chips, las dos ventanas (un producto ya conocido cuya reposición era ANTERIOR al último pedido cae de la lista, y uno nunca visto de la misma fecha se queda) y los tres filtros, más el circuito completo en pantalla.'],
              ['**21/8/2026**', '**LA SUCURSAL LA PONE EL EQUIPO: se registra la terminal y el login deja de preguntar** (migraciones 0081 y 0082). El desplegable de sucursales del login venía **precargado con la primera de la lista** —la Distribuidora—, así que la cajera de Express 2 que no lo tocaba entraba en la Distribuidora **sin haber elegido nada** y vendía descontando stock del local equivocado. Y no lo detecta nadie: el arqueo del cierre da cero igual porque es coherente consigo mismo; lo que queda roto es el stock de dos locales y plata física anotada en otro lado, y eso aparece semanas después. Desde que ARCA está andando es peor: cada local tiene su punto de venta con numeración propia y su domicilio impreso, así que una factura mal emitida **consume un número que no vuelve y sale con el domicilio de otro local**, con CAE y sin poder borrarse. **La solución no es avisar mejor: es que no haya nada que elegir.** Ahora un jefe registra la máquina una vez desde **Sistema › Este equipo** ("Caja 1", y en qué sucursal está); queda un token en el `localStorage` de ese navegador, y desde ahí el login de esa PC **muestra "Caja 1 · Express 3" fijo, sin desplegable** — la cajera pone quién es y su clave, nada más. **El candado vive en el servidor**: con terminal registrada el `sucursalId` que venga en el body **se ignora**, así que no se puede entrar en otro local ni con un POST a mano. Se ignora también para el jefe, a propósito: él cruza sucursales con el selector del encabezado, que ya existe y deja rastro. **Cuatro decisiones que vale entender.** (1) **Se ata al EQUIPO y no a la persona** porque las cajeras rotan: asignarles una sucursal obligaría a reasignarlas todos los días, mientras que la PC de Express 2 está siempre en Express 2 — se ata el dato a lo que está clavado, no a lo que se mueve. (2) **Se descartó deducirla por IP** (que era la opción más elegante) porque cuando se corta internet las cajeras siguen vendiendo con los datos del celular: la IP pasa a ser la del operador, cambia sola y dos locales pueden verse iguales — y el aviso saltaría justo los días de más quilombo, que es la forma más rápida de enseñar a ignorar los avisos. El token de la terminal, en cambio, **vive en el navegador y no en la red**: anda igual con fibra, con el repetidor o colgado de un celular. (3) **Leer el nombre de la PC es imposible** y no hay truco: el navegador no expone hostname, serial ni MAC. Así que el nombre se lo ponemos nosotros, que además sirve más ("Caja 1" contra "DESKTOP-8F3K2L"). (4) **Registrar un equipo es de administración** (`sistema.terminales`, solo admin): si lo pudiera hacer el mostrador, el arreglo no serviría para nada — sería el desplegable con un paso más. **El token no es una credencial de privilegio**: lo único que decide es dónde se para el que entra, y eso hasta ayer era libre; probado que **no sirve para autenticarse** (da 401). Y si se borra el `localStorage` no se rompe nada: el login vuelve a preguntar y se registra de nuevo. **Aparte, el default que causaba todo: ya no hay ninguna sucursal preseleccionada** cuando el equipo no está registrado — un desplegable precargado invita a no mirarlo, uno vacío obliga a elegir. **Probado**: 16 comprobaciones por API (la que importa, "el body pidió la Distribuidora y el servidor puso Express 3", en verde) y el circuito completo en pantalla — registrar, salir, y el login mostrando el equipo fijo y entrando en Express 3, verificado en la fila de `sesiones`.'],
              ['**21/8/2026**', '**"NO LLEGÓ LA RESPUESTA" DEJÓ DE SIGNIFICAR "LA VENTA NO SE HIZO".** El navegador corta a los 20 s (`timeoutMs`) y facturar llama a ARCA, donde pedir un ticket de acceso nuevo ya son unos 10: colgada de los datos del celular, pasarse es normal. Cuando eso pasa el navegador corta pero **el servidor no se entera y termina la venta igual**, con CAE incluido si alcanzó a pedirlo. Y hasta hoy ese caso mostraba **el mismo cartel que un rechazo de verdad** ("No se pudo registrar la venta"), porque un abort rechaza con `AbortError`, que no es `HttpError`, así que `e.data.message` venía vacío y caía en el genérico — un corte de red daba exactamente lo mismo. La cajera no podía distinguir "no salió" de "salió y no me enteré", y con el cliente enfrente lo natural es rehacerla: **venta duplicada, stock descontado dos veces y dos CAE para la misma compra**. La duplicación no era técnica sino **humana, inducida por el mensaje**. Ahora `httpClient` marca `sinRespuesta` (solo cuando el fetch rechaza, que es solo cuando no hubo respuesta) y el POS **no adivina: le pregunta al servidor cómo quedó el ticket**, y pregunta **cuatro veces cada 2 s** — porque que el navegador haya cortado no significa que el servidor haya terminado, y preguntando una sola vez se vería "borrador" y se diría "no salió" justo antes de que salga. Los tres finales se dicen con todas las letras y solo uno pide rehacer el cobro. De paso, `configImpresion()` es OTRA llamada HTTP que puede fallar sola **después** de una venta bien registrada: eso caía en el mismo `catch` y también decía que la venta no se había registrado; ahora un problema de impresora dice que es un problema de impresora. **Probado en el POS de verdad**, con el cobro frenado a mano reteniendo la fila de la caja desde psql: en la traza de red se ve el `POST /confirmar` con **`net::ERR_ABORTED`** y las **cuatro consultas** que siguen, y en pantalla salió el aviso correcto. Después, con la venta ya cerrada por el servidor, **el reintento se rechazó sin duplicar nada** — para eso se le puso a `confirmar` un mensaje propio en vez del de `exigirBorrador` ("no se puede modificar"), que es el correcto para editar un ticket y el peor posible cuando lo que hay que decir es qué hacer. **Y el caso del final feliz también**, que es el que exige que el servidor termine DENTRO de la ventana de 8 s en que el POS pregunta: con el timeout de 20 s esa coincidencia no se podía acertar a mano, así que se bajó a **3 s un rato** (y se devolvió a 20 000 al terminar) y ahí salió redondo, **dos veces seguidas**: el `POST` figura **abortado** en la traza de red y en pantalla igual aparece **"Venta registrada"** con su comprobante — a esa pantalla solo se llega por el camino de recuperación. Dato de método: **marcar la fila desde afuera para simularlo NO sirve**, el `UPDATE` queda bloqueado porque el cobro en vuelo ya tiene la venta tomada con candado. Es el arreglo de más abajo funcionando, y de paso la prueba de que los dos se sostienen entre sí.'],
              ['**21/8/2026**', '**EL MISMO CANDADO, DOS AGUJEROS MÁS: el turno que se cierra en el medio, y anular dos veces.** Salieron mirando el arreglo anterior. (1) **`confirmar` resolvía el turno de caja FUERA de la transacción** y adentro le grababa el `cajaSesionId` sin volver a mirarlo. Si alguien cerraba la caja en ese hueco —que incluye los 15 s de ARCA—, la venta se colgaba de un turno **ya cerrado**: su plata entraba a un arqueo que se había calculado y firmado sin ella, o sea que **quedaba fuera de todo arqueo** y nadie se enteraba nunca. Es el agujero que `caja.module.ts` ya había cerrado para los movimientos manuales y las cobranzas: su comentario nombra "las dos puertas que insertan movimientos", y **las ventas no eran ninguna de las dos**. Con **un solo cajón compartido por dos cajeras** (lo que se confirmó el mismo día) dejó de ser raro: una cierra mientras la otra cobra. (2) **`anular` no tomaba la venta con candado**, y anular DEVUELVE STOCK: seis anulaciones simultáneas de una venta de 2 unidades dejaron el stock en **202 donde había 192** — diez unidades de mercadería inventada. Es `confirmar` al revés: allá la mercadería salía de más, acá entraba. Su chequeo de "el arqueo ya está firmado" también corría afuera, así que se le puso el mismo par de candados. **Las dos pruebas se corrieron en los dos sentidos.** La del turno no se puede provocar apretando botones (la ventana dura milisegundos): se forzó **reteniendo la fila de `caja_sesiones` desde psql** con `FOR UPDATE` y cerrándola justo antes de soltarla — el cobro esperó 6,4 s en el candado y rechazó; sin el candado, la misma prueba dejó la venta **confirmada y colgada de un turno cerrado**. Detalle que apareció de paso: el pedido se frena igual aunque falte el candado, porque grabar `caja_sesion_id` toma un lock de clave foránea sobre esa fila — pero después escribía igual, que es lo que importaba. **Orden de los candados en los cuatro caminos: primero la venta, después el turno**; dos caminos que toman los mismos dos candados en orden distinto se traban entre sí.'],
              ['**21/8/2026**', '**EL CANDADO DE LOS TRES CAMINOS QUE EMITEN COMPROBANTES, y una prueba que dio 6 ventas donde había una.** `confirmar()`, `facturarAhora()` y `notaCredito()` leían **fuera de la transacción** el estado que los habilita y escribían adentro. Entre esas dos cosas pasan segundos —pedirle el CAE a ARCA puede tardar 15— y en ese hueco entraba un segundo pedido. **El caso real no es el doble clic**: es la cajera colgada de los datos del celular que reintenta porque no le llegó la respuesta, y las dos cajas del local cobrando a la vez. **Medido, no estimado**: seis confirmaciones simultáneas del mismo ticket de 2 unidades, contra el código viejo, dieron **6 ventas aceptadas y 12 unidades descontadas de stock** (y 6 pagos en el cajón); con el candado, **1 aceptada, 5 rechazadas y 2 unidades**. La prueba se corrió en los dos sentidos a propósito: si no se verifica que el test SEPA fallar, no prueba nada. La pieza es `bloquearVenta()`, que toma la fila con `FOR UPDATE`, pero **lo que arregla no es tomar el candado sino volver a validar después de tomarlo** — el estado leído antes de la transacción ya no vale, y esa relectura es la que rechaza al segundo. En la nota de crédito el candado va sobre una fila que no se escribe: **la venta original**, porque su saldo acreditable es un recurso compartido entre todas sus notas, y ahí se rehacen adentro las tres cuentas (el tope en plata, el tope por renglón y la plata que queda para devolver del cajón). Era **bloqueante del certificado de producción**. Lo que NO cubre: si dos pedidos llegan justo juntos, los dos ya le pidieron el CAE a ARCA antes del candado — pero los dos piden el MISMO número (los dos consultaron el último autorizado antes de que ninguno emitiera), así que ARCA rechaza el segundo por duplicado. Queda del lado de ARCA, no del nuestro.'],
              ['**21/8/2026**', '**TRES AGUJEROS QUE DESTAPARON LOS AGENTES DE AUDITORÍA, y los tres estaban alrededor de la nota de crédito.** (1) **El candado vivía en el navegador.** El botón se le escondía al cajero con `esJefe`, pero el endpoint pedía `devoluciones` — que **el cajero tiene** desde la 0024. Un POST lo salteaba y emitía una nota de crédito de cualquier venta de la sucursal. Ahora hay una acción propia, **`nota_credito` (migración 0080), solo para el administrador** por decisión del dueño, y la pantalla mira esa misma llave. **`devoluciones` sigue siendo del cajero** porque es la que habilita ANULAR, que es otra cosa: anular deshace un comprobante NUESTRO que nunca salió a ARCA; la nota de crédito emite uno FISCAL. (2) **La devolución en efectivo no miraba si la venta se había cobrado.** Sacaba del cajón el total de la nota, sin más. El circuito que lo vuelve grave: una venta en cuenta corriente que el cliente nunca pagó, nota de crédito con devolución, y sale la plata — **el arqueo cierra perfecto** (el egreso quedó registrado) y la deuda se borra con la nota, así que no queda a quién reclamarle. Ahora hay tope: no se devuelve más de lo que entró, y **lo que entró son DOS cosas** —los pagos del mostrador y las cobranzas imputadas—; mirar solo las cobranzas habría bloqueado la devolución de cualquier venta de contado, que es el caso normal. (3) **"Facturar" convertía una nota de crédito en factura.** Una nota que quedaba sin CAE caía en ⚠ Sin facturar con su botón, y ese botón resuelve la letra de una FACTURA: apretado ahí emitía una **Factura B con CAE real** por el importe que había que devolver, y un comprobante fiscal no se borra. Es la misma familia del candado del 20/8 (el reintento convertía el ticket provisorio en factura sin CAE): ese ya estaba, este faltaba. Ahora la API lo rechaza y el botón no se ofrece — la nota se sigue VIENDO, porque quedó sin CAE y hay que saberlo, pero **su reintento todavía no está construido**.'],
              ['**21/8/2026**', '**CUATRO COMPROBANTES QUE ANTES NO EXISTÍAN EN PAPEL**, y la reimpresión de ventas movida a donde se ve. Se agregaron el **remito de una transferencia** (en la ficha del envío), el **vale de una operación** (Almacén › Operaciones), el **comprobante de un gasto** y la **orden de pago** a un proveedor (Gastos, en sus dos pestañas). Los cuatro se eligen su formato en Sistema › Impresión como cualquier otro documento. **Todos llevan al pie cuándo se imprimieron y quién**, y eso no es adorno: una reimpresión NO sale en el momento del hecho, así que un remito encontrado tres meses después no se distingue de uno emitido el día de la recepción — y si la transferencia cambió de estado en el medio, el papel viejo miente. **Ninguno inventa datos**: se arman con lo que la pantalla ya tiene a la vista, porque un impreso que muestre algo que la pantalla no muestra es una segunda fuente de verdad y a la larga divergen. Tres decisiones que valen: el **remito lleva las tres cantidades** (pedido, enviado, recibido) porque la diferencia entre ellas ES el documento —pedido contra enviado dice qué no había en el depósito, enviado contra recibido qué se perdió en el camino, y con una sola columna no se puede reclamar nada—; la **orden de pago va en A4 aunque salga de la caja**, porque es el único de los cuatro que se le entrega a alguien de afuera y un rollo térmico se borra en unos meses; y el **vale se ofrece en TODAS las filas**, no solo en las bajas, porque la regla "solo merma y ajuste" obliga a explicar por qué esta fila tiene botón y la de al lado no. Aparte: el botón de **reimprimir una venta** existía desde el 19/8 pero vivía en el pie de la ficha, el mismo lugar donde la nota de crédito no se encontraba; ahora está en la fila del listado.'],
              ['**20/8/2026**', '**LA PRIMERA FACTURA REAL (homologación), y el control que la dejó pasar sin CUIT.** Se emitió el circuito completo contra el ARCA de homologación desde el servidor de producción: **Factura B 00028-00000001 con CAE y QR**, y su **Nota de crédito B 00028-00000001** citando el comprobante asociado — cada una con su numeración, que empieza en 1 por separado porque son tipos distintos (cód. 06 y 08). Funcionó todo… menos el papel: **la factura salió SIN el CUIT del emisor** y a nombre de "Sabor y Aroma" en vez de la razón social. La causa es que Sistema › Empresa está vacío EN PRODUCCIÓN (se había cargado en desarrollo), y la plantilla omite la línea del CUIT cuando el dato no está. **Lo grave es que nada avisó**: `diferenciasConEmpresa` comparaba el CUIT del certificado contra el del membrete **exigiendo que los dos existieran**, así que "no cargado" pasaba en silencio — el control fallaba justo en el caso que más importa. Y el CAE se pide con `ARCA_CUIT`, o sea que ante ARCA el comprobante estaba bien: **el error vivía solo en el impreso**, que es donde nadie mira hasta que lo mira un inspector. Ahora el panel avisa de las dos cosas: CUIT vacío ("las facturas se imprimen SIN el CUIT del emisor") y razón social vacía ("van a salir a nombre de …, que es el nombre de fantasía"). Probado en los cuatro casos: todo vacío da 2 avisos, falta la razón social da 1, CUIT de otro da el de siempre, y completo no da ninguno.'],
              ['**20/8/2026**', '**EL NOMBRE Y LA RAZÓN SOCIAL SON DOS CAMPOS, y los datos fiscales pasaron a ser los reales.** Sistema › Empresa tenía **un solo** "Nombre de la empresa", y lo usaban por igual el membrete y la factura. Acá eso no alcanza: el negocio se conoce como **Sabor y Aroma** y quien factura es **LORENZO LUCAS EMANUEL**. Poner la razón social en ese campo único habría cambiado el membrete de todos los tickets; dejarla afuera habría emitido facturas sin el dato que la **RG 1415 exige**. Así que ahora son dos: el de fantasía va grande en el membrete, y la **razón social** sale como **Emisor** de la factura, prellena el pedido del certificado de ARCA y, si es distinta del nombre, se agrega a la línea de datos del membrete — **si son iguales no se repite**, que en un rollo de 58 mm es una línea que se agradece. Vacía cae al nombre, así que un unipersonal donde coinciden no tiene nada que cargar. Y se cargaron los datos verdaderos: CUIT **23-35678242-9**, razón social **LORENZO LUCAS EMANUEL**. Con eso se apagaron dos falsos avisos que venían sonando siempre: el de la bandeja de facturas ("esta factura no es nuestra", en TODAS) y el del panel de ARCA (el CUIT del certificado contra el del membrete). Un aviso que suena siempre enseña a ignorar los avisos. **La trampa del camino**: la configuración pasa por `sanitize`, que **descarta toda clave que no esté en los defaults** — un campo nuevo que no se agregue ahí se guarda, vuelve vacío y nadie entiende por qué. **Falta todavía el domicilio fiscal**: la dirección y el teléfono siguen siendo los de ejemplo y se imprimen en el membrete de todo.'],
              ['**20/8/2026**', '**EL TRÁMITE DEL CERTIFICADO, DESDE LA PANTALLA.** El paso que no se hace en la web de ARCA: generar una **clave privada** y un **pedido** (`.csr`), subir el pedido, y guardar el certificado que devuelven. Se hacía con dos comandos de `openssl` a mano y ahí se pierde media tarde — probado en la máquina del dueño, **`openssl` falla de dos maneras distintas** y ninguna habla de certificados: `OPENSSL_CONF` apunta a un archivo que dejó PostgreSQL y no existe, y Git Bash convierte el `/C=AR/...` en una ruta de Windows. Ahora son tres botones en **Ventas › Configuración**. **LA CLAVE PRIVADA NO PASA POR EL NAVEGADOR**: se genera en el servidor y se escribe donde apunta `ARCA_KEY_PATH`, con permisos 0600; de vuelta viene solo el `.csr`, que es público —está hecho para dárselo a ARCA—. Una clave que viajara terminaría en el historial del navegador, en la carpeta de Descargas de una máquina del mostrador y en cualquier proxy del camino. **NUNCA PISA UNA CLAVE EXISTENTE**: si ya hay una, el pedido se arma con ESA. Sobrescribirla dejaría muerto en el acto al certificado que le corresponde y no hay vuelta atrás, porque la clave vieja no se recupera; para forzar una nueva hay que borrar el archivo en el servidor, y es incómodo a propósito. **Al instalar el `.crt` se controlan tres cosas**, y las tres evitan un fallo que después aparece disfrazado de otro —el WSAA solo dice "Certificado bloqueado"—: que **se corresponda con la clave** (el error más fácil de cometer: el de homologación pegado en producción), que sea **del CUIT nuestro**, y que **no esté vencido**. El sistema lo ve **sin reiniciar**. La pantalla también avisa cuando al certificado le quedan menos de 30 días: renovarlo es un trámite, no un botón. **Verificado con 17 pruebas**, incluidas las que importan: la clave no aparece en ninguna respuesta, **`openssl` lee el pedido que arma el sistema** con el `serialNumber` exacto y le verifica la firma, la clave existente queda byte por byte intacta, los cuatro certificados malos se rechazan **sin escribir nada en el disco**, y el circuito completo hecho a mano en pantalla: generar → copiar → pegar el `.crt` → "Certificado instalado, vence el 19/8/2028". **Un bug que apareció probándolo a mano**: apretar Generar creaba la clave y **borraba de la pantalla el pedido que había que copiar** — el panel se recargaba y el cartel de "Leyendo…" desmontaba todo. Ahora ese cartel sale solo la primera vez.'],
              ['**20/8/2026**', '**UN PUNTO DE VENTA POR SUCURSAL (migración 0077).** ARCA declara los puntos de venta **contra un domicilio**, y cada uno lleva su numeración correlativa independiente. Sabor y Aroma tiene cinco, todos bajo el mismo CUIT: **00028** la Distribuidora (28 de Junio 85, venta al por mayor), **00029** Fontana (Pringles 808), **00030** Express 1 (Belgrano 728), **00031** Express 2 (Av. 9 de Julio 1291) y **00032** Express 3 (Av. González Lelong 1501). Hasta hoy el sistema tenía **uno solo** para toda la empresa. Ahora la sucursal lleva el suyo —y **su domicilio**— y se cargan en **Gerencia › Usuarios y roles › Sucursales**; los números no van en el código porque son datos de la empresa, no del programa (una migración que los escribiera los inventaría en cualquier base nueva). **Un solo certificado alcanza para los cinco**: el certificado autoriza a un CUIT a usar el webservice, no a un local. **CINCO DÍGITOS, y era un error de verdad**: el punto de venta se formateaba a cuatro. Con el 0001 de la etapa de prueba nunca se notó, pero **ninguno de los cinco reales entra en cuatro** — la factura habría dicho `0028-00000001` mientras ARCA registra `00028-00000001`, que es justo el número por el que se lo coteja contra Mis Comprobantes. **El domicilio de la factura es el del local que la emitió**, no uno solo de la empresa: la de Belgrano 728 tiene que decir Belgrano 728. **Tres cosas que se resolvieron de paso**: dos sucursales **no pueden compartir** punto de venta (índice único + mensaje que explica por qué: las dos le pedirían el mismo número a ARCA y una rebotaría); la cadena que serializa las emisiones pasó a llevar el punto de venta en la clave, así que Fontana y la Distribuidora **facturan a la vez sin esperarse**; y **la nota de crédito sale por el punto de venta de la factura que ajusta**, no por el de la sucursal de hoy — si la venta fue en Fontana y la devolución la toma la Distribuidora, la nota sigue siendo de Fontana, que es la única forma de que el comprobante asociado cierre. El panel de diagnóstico muestra ahora **una fila por local** con su punto de venta, su domicilio y su último número autorizado, y las consulta **en paralelo** (en serie, cinco locales serían veinte segundos de botón apretado). **Verificado**: los cinco cargados y normalizados por la API (se tipea "29" y queda "00029"), el duplicado rechazado sin tocar la otra sucursal, la venta llevando el domicilio de su local al impreso, y —con el `fetch` interceptado y un ticket de acceso de prueba— **el sobre SOAP saliendo con el punto de venta correcto en las tres llamadas** (último número, pedido de CAE y consulta de recuperación), sin arrastrar el de la llamada anterior.'],
              ['**20/8/2026**', '**EL PANEL DE DIAGNÓSTICO DE ARCA (fase 6 de 8), y dos agujeros que destapó.** El panel vive pegado al interruptor de facturación, en **Ventas › Configuración**, porque son las dos mitades de lo mismo: el interruptor es la **intención** (quiero facturar) y esto es la **capacidad** (puedo). Muestra el entorno —**PRODUCCIÓN en rojo**, porque es la única diferencia visible entre ensayar sin consecuencias y emitir facturas de verdad—, el CUIT del certificado, el punto de venta de web services, y **qué falta exactamente** cuando falta algo (las cinco causas con nombre y apellido, no un "no disponible" pelado). El botón **Probar conexión** hace las tres preguntas del §2.4 **en orden y cortando en la primera que falla**: seguir preguntando después de un certificado rechazado da tres errores que dicen lo mismo y esconden cuál era la causa. La primera pregunta —¿ARCA está vivo?— **corre aunque no haya certificado**, porque `FEDummy` no lleva credenciales: es justo la que contesta si el problema es de ellos o nuestro. Abajo, las **ventas trabadas** con su motivo y su botón de reintento, ordenadas de la más vieja primero. **Son dos consultas separadas y no una**: el estado se lee del entorno y del disco (instantáneo, se pide al abrir la pantalla) y la prueba sale a la red — **medido, 10,2 segundos**, que es lo que cuesta un ticket de acceso nuevo. Fundirlas habría hecho que abrir Configuración se cuelgue diez segundos contra un servicio ajeno. **PRIMER AGUJERO, y es el que más hubiera dolido**: `ARCA` leía `process.env` **al importar el módulo**, y `ConfigModule` carga el `.env` después. En el servidor funciona igual (ahí las variables las inyecta el contenedor antes de que Node arranque), pero en la máquina de desarrollo, con las cinco variables escritas en el `.env`, la facturación quedaba muda y este mismo panel iba a insistir con "Falta ARCA_CUIT" mientras el archivo lo tenía escrito. Ahora se lee con getters: cada consulta mira `process.env` en el momento. **SEGUNDO AGUJERO**: apretar **Facturar** sobre un ticket provisorio con ARCA **apagado** lo convertía en `factura_b` **sin CAE**, le borraba el pendiente y lo sacaba de la lista — los tres papeles dejaban de coincidir de golpe (el cliente con un ticket que dice "provisorio", el sistema diciendo Factura B 0001-00000001, y ARCA sin nada), y la venta ya no aparecía en ningún lado para arreglarla. El candado viejo miraba el tipo y no cubría el caso; ahora la regla es la que corresponde: **sin CAE no hay factura**, y la venta se queda pendiente. **Verificado contra el ARCA de homologación real**: sin certificado dice "ARCA está funcionando; lo que falta es de este lado" (270 ms); con un certificado autofirmado, "El certificado autentica (WSAA): NO — Certificado bloqueado" a los 10.242 ms; el aviso de CUIT distinto al del membrete salta; y el reintento sobre dos ventas trabadas de verdad falla con su motivo y las deja donde estaban.'],
        ...
        bloques: [
          {
            ...
            filas: [
              ['**Tipo de movimiento `venta_unidad` propio para los enteros** (27/8/2026 — mientras tanto, arreglado en pantalla)', 'La API guarda la venta de un producto entero como `venta_fraccionada` (solo distingue "granel suelto" de "todo lo demás" — herencia del modelo original). El 27/8 se corrigió **lo que se muestra**: donde el producto es entero, la pantalla dice "Venta por unidad" en historial, vale, Dashboard y ficha. Lo que falta es que **el dato diga la verdad**: agregar `venta_unidad` al enum `tipo_movimiento` + backfill de las filas existentes de enteros (`UPDATE movimientos SET tipo = ... WHERE tipo = \'venta_fraccionada\' AND producto entero`). Ojo con la trampa conocida: agregar un valor a un enum con migrate corriendo en una sola transacción obliga a recrear el enum. Conviene hacerlo colgado de otra migración, no como despliegue propio. Al hacerlo, el arreglo de pantalla (labelTipoMov en constants.js) queda obsoleto y se simplifica.'],
              ['**Se corta internet y la caja se cuelga del celular de la cajera** (21/8/2026, salió de costado preguntando por las conexiones — **NO ESTABA ANOTADO EN NINGÚN LADO**)', 'Dato nuevo del dueño: **la conexión de cada local es distinta**, en Distribuidora hay un **repetidor**, y cuando se corta internet **las cajeras se cuelgan de los datos de su propio celular para poder seguir vendiendo**. Eso ya pasaba con el sistema viejo, así que **no es una regresión** —los dos son online— pero conviene tenerlo escrito porque toca varias cosas a la vez. **Lo que confirma**: el sistema no puede depender de la red para saber dónde está parado alguien (mató la opción de la IP en la entrada de abajo). **Lo que hay que mirar**: la continuidad hoy depende de que la cajera **tenga datos, tenga batería y tenga ganas** — es un plan de contingencia que no es de la empresa sino de la empleada, y el día que no lo tenga el local no factura. La mejora más barata no es código: **un módem 4G propio por local como respaldo**, y que el celular deje de ser el plan B. **VERIFICADO el 21/8, y el resultado es mitad y mitad.** La pregunta era si se duplica una venta cuando la conexión se corta al confirmar. **Sola no se duplica**, y hay tres capas que lo impiden: `httpClient` **no reintenta nunca** (no hay retry automático), el confirmar apunta a un **borrador que ya existe** (el POS abre la venta primero y después la confirma, así que un reintento va contra el MISMO id y `exigirBorrador` lo rechaza), y el doble clic está frenado por `!enviando`, que cubre también F10 y F8. **Pero hay un hueco, y es justo el caso del celular.** El cliente **aborta a los 20 segundos** (`timeoutMs` en `app.config.js`), y facturar llama a ARCA — pedir un ticket de acceso nuevo ya son unos 10 s. Cuando se pasa de 20 s el navegador corta pero **el servidor no se entera y termina la venta igual**, con CAE incluido si alcanzó a pedirlo. La cajera ve "No se pudo registrar la venta", **el mismo texto que un error de verdad**: un abort rechaza con `AbortError`, que no es `HttpError`, así que `e?.data?.message` viene vacío y cae al mensaje genérico — y un corte de red da exactamente lo mismo. **No tiene forma de distinguir "no salió" de "salió y no me enteré".** Si reintenta sobre el mismo ticket está a salvo (aunque le aparece "La venta ya está emitida" justo después de "no se pudo registrar", que suena a contradicción); **pero si lo rehace como ticket nuevo —lo natural con el cliente enfrente— ahí sí hay venta duplicada, stock descontado dos veces y, si era factura, DOS CAE para la misma compra**. O sea: **la duplicación no es técnica, es humana, y el sistema la induce** al no decirle qué pasó. **El arreglo es chico y no toca la lógica de venta**: distinguir el abort y el corte de red de un rechazo del servidor, y ahí decirle "no llegó la respuesta — fijate en Ventas si salió antes de rehacerla", o mejor, re-consultar el estado del ticket y resolverlo solo'],
              ['**Dos cajeras en un local, un solo turno de caja** (21/8/2026 — **RESUELTO EL MISMO DÍA: queda como está**, probado por el dueño)', '**La respuesta fue que en el local hay UN solo cajón físico**, compartido por las dos cajeras. Con eso el modelo actual es el correcto y no hay nada que construir: un cajón, un turno, un arqueo. El dueño lo probó entrando con **dos cajeras a la vez en la misma sucursal** y haciendo ventas con las dos: **el listado de ventas muestra el nombre de cada una**, porque `ventas.usuarioId` guarda quién vendió aunque el turno sea compartido. O sea que se sabe qué vendió cada una, que era lo que importaba. Lo único que sigue sin poder responderse es **de quién fue un faltante**, y eso es inherente a compartir un cajón: ningún sistema lo puede saber si las dos meten la mano en el mismo lugar. **Si algún día se pone un segundo cajón, esto se reabre** y ahí sí el turno tendría que pasar a ser por terminal. Queda anotado el análisis original por si eso pasa: la invariante está escrita en el encabezado de `caja.module.ts` y `abrir` la hace cumplir — **una sola sesión abierta por sucursal**. Eso se leyó al principio como un problema —dos cajeras cobrando a la vez en Distribuidora y en Fontana, un solo turno— **suponiendo dos cajones**: ahí el arqueo del cierre habría sumado dos cajones en uno, el `montoInicial` habría sido uno solo para dos fondos, y un faltante no habría tenido dueño. **Con un solo cajón nada de eso aplica**, y la segunda cajera entrando sobre el turno ya abierto es el comportamiento correcto, no un síntoma. Si mañana hay dos cajones, lo que hace falta ya está a mitad de camino: `ventas.usuarioId` y `cajaMovimientos.usuarioId` existen, así que **la plata ya se puede atribuir por persona** y lo único que faltaría es que el TURNO sea por cajón — la forma natural de nombrar el cajón es la **terminal** de la entrada de arriba. **Sería el único de los cambios propuestos que toca datos existentes** (habría que migrar los turnos abiertos de producción). **Lo que NO se resuelve con un cajón compartido**, y sigue en pie: las dos cajeras facturan por el **mismo punto de venta**. Para ARCA eso está perfecto (un punto de venta admite varias terminales y la numeración la lleva ARCA), pero del lado nuestro es **exactamente la carrera del `FOR UPDATE`** — con una cajera por local era teórica, con dos cobrando en simultáneo era cuestión de tiempo que dos ventas se llevaran el mismo número, y eso es independiente de cuántos cajones haya. **Arreglada el mismo 21/8** (ver el registro): fue justamente esto lo que la sacó de la lista de "algún día"'],
              ['**Reintentar una nota de crédito que quedó sin CAE** (21/8/2026)', 'Si ARCA no contesta al emitir una nota, queda con `facturarPendiente` y **hoy no tiene camino de reintento**: se ve en ⚠ Sin facturar y en el panel de ARCA, pero el botón "Facturar" está deshabilitado a propósito (21/8) — ese botón resuelve la letra de una FACTURA y la convertiría en Factura B con CAE real. Hace falta un circuito propio, y antes una decisión: qué pasa con la nota que quedó a medio emitir si ARCA en realidad sí la registró'],
              ['**Decisión del dueño: ¿quién puede ANULAR una venta?** (21/8/2026)', 'La nota de crédito quedó resuelta el 21/8 (solo administración, migración 0080). Anular sigue con el mismo desfasaje que tenía aquella: la pantalla lo muestra solo al jefe (`esJefe`) y la API pide `devoluciones`, **que el cajero tiene**. No se tocó porque cambiarlo en cualquiera de los dos sentidos es una decisión de negocio, no una corrección: alinear la pantalla a la API le ABRE la acción al cajero; alinear la API a la pantalla se la saca. Hoy el que manda es el permiso, así que el cajero puede anular por la API aunque no vea el botón'],
              ['**Facturación ARCA — pasar a producción (fase 8)** (20/8/2026 — **VA ÚLTIMO, por decisión del dueño el 21/8**: primero se prueba todo lo demás)', '**No está frenado por falta de nada: está frenado a propósito.** El dueño decidió el 21/8 dejar el pasaje a producción **para el final**, después de probar el resto del sistema contra homologación, que es donde equivocarse no cuesta nada. Es la decisión correcta y conviene tenerla escrita, porque de acá en adelante todo lo que salga con CAE de producción **ya no se borra**: se corrige con otra nota de crédito y queda en el libro de IVA. Ya está TODO construido y **probado contra homologación desde el servidor**, factura y nota de crédito con CAE incluidas (20/8/2026). Cuando llegue el momento falta: el certificado de **producción** —otro trámite, en Administración de Certificados Digitales—, `ARCA_ENV=produccion`, y el §12 de la guía: **dejar ⚠ Sin facturar en cero ANTES de prender el interruptor** y hacer la primera factura real a mano y mirándola. Ver "Facturar con ARCA, paso a paso"'],
              ['**Nota de DÉBITO de venta** (20/8/2026)', 'La de crédito ya está (20/8, migración 0076). Falta la de débito, que es lo que corrige una nota de crédito mal emitida o cobra algo que faltó facturar. El enum de la base ya tiene los tres tipos (`nota_debito_a/b/c`) — se agregaron en la misma migración porque recrear el enum es la parte cara; falta el circuito'],
              ...
              ['**Deploy: las cuatro contraseñas `1234` de la semilla** (14/8/2026, BLOQUEANTE del primer deploy)', 'La semilla crea cuatro usuarios con la contraseña **`1234`**, y uno de ellos —`Lucas`— es **superadmin**, o sea que puede todo en todos los módulos. Mientras el sistema vivía en la red del local eso era una molestia; con el dominio publicado en internet es la puerta de entrada más barata que existe, y el nombre de los usuarios se ve en el desplegable del propio login. Dos caminos, los dos válidos: **no correr la semilla en producción** (cargar la empresa a mano desde cero), o correrla y **cambiar las cuatro desde Gerencia › Usuarios y roles ANTES de apuntar el dominio**. Desde el 16/9 el mínimo volvió a **4** (puede ser un PIN), así que `1234` hoy es una contraseña que el sistema acepta: lo que la hace peligrosa no es el largo sino que **viene de fábrica, es la misma en las cuatro cuentas y está escrita en este manual**. Cambiarlas sigue siendo obligatorio. Está primero en el checklist del `DEPLOY.md`'],
              ...
              ['**El sugerido de los días sin pedido**', 'La primera mitad de esta ficha —que Fraccionar llegue con lo que falta y vuelva al pedido— **se hizo el 17/9/2026**, y de paso confirmar la lista arma el faltante solo. Lo que queda es el otro lado: para los días **sin pedidos**, `stockMin` ya alimenta el sugerido de transferencias, y la misma cuenta sobre las presentaciones diría "faltan 30 paquetes de 500 g en Express 2" en vez de que el fraccionador elija de memoria'],
        ...
        bloques: [
          {
            t: 'lista',
            items: [
              '**🟠 EL STOCK DE PRODUCCIÓN ES DE PRÁCTICA — LIMPIARLO ANTES DEL CORTE REAL (cargado el 28/8/2026).** Para que el equipo se entrene vendiendo y pidiendo, se cargaron por única vez **50 unidades/kg de cada producto en Distribuidora** (2.757 ajustes vía la API real, motivo "Carga inicial para pruebas del sistema (28/8/2026) — no es mercadería comprada", todos visibles en el historial de movimientos). Consecuencia: **la valuación del inventario, Gerencia y los reportes muestran números de práctica**, y las ventas/movimientos que el equipo genere probando también son de práctica. **El día del corte real**: (1) descargar un respaldo y apretar el botón **"Fin del período de prueba"** de Sistema › Respaldos (construido el mismo día, solo superadmin: vacía TODA la operatoria de práctica conservando catálogo, proveedores y clientes), (2) el stock verdadero entra por **Control de stock** (conteo ciego por sucursal) o por las primeras facturas reales. Hasta entonces, no leer los números de Gerencia como reales.',
              '**RESUELTO el 25/8/2026 — fotos de la tienda en LOTE, las dos piezas construidas** (ver el registro del día). Quedan las dos notas de vigilancia del análisis: las fotos viven en la base (base64, ~20 MB con 200 productos — bien hoy, revisar si el catálogo web crece a miles) y hay UNA foto por producto (índice único: una galería sería migración).',
              '**RESUELTO el 25/8/2026 — el turno de la cobranza lo resuelve el servidor Y con candado.** La primera mitad ya estaba (el 22/8 el alta pasó a resolver el turno por sucursal, ignorando el body); el 25/8 se cerró la carrera: dentro de la transacción el turno se retoma FOR UPDATE y se revalida abierto — con efectivo obligatorio se rechaza si cerró en el medio, sin esa exigencia el recibo cae a administración',
              '**RESUELTO el 25/8/2026 — anular venta Y cobranza miran el turno, con candado.** La venta desde la mañana; a la tarde la cobranza ganó lo mismo: la anulación corre en transacción con la cobranza FOR UPDATE (dos anulaciones a la vez: gana una) y el turno FOR UPDATE (un cierre en curso espera o la anulación se rechaza porque el arqueo ya se firmó, salvo jefe)',
              '**RESUELTO el 25/8/2026 — la venta toma el turno de caja CON candado**: `confirmar()` bloquea la fila del turno FOR UPDATE dentro de la transacción y revalida que siga abierto (rechaza con "El turno de caja se cerró mientras se cobraba este ticket"). La carrera del cierre quedó cerrada también acá, como ya lo estaba para egresos y movimientos manuales.',
              '**RESUELTO el 25/8/2026 — una sola caja abierta por sucursal, garantizado por la base.** Índice único PARCIAL `uq_caja_abierta_por_sucursal` (migración 0085: solo cuenta las abiertas) + el alta atrapa el 23505 y contesta el mensaje amable. Probado con dos aperturas en paralelo: una entra, la otra recibe «Ya hay un turno abierto». La migración además cierra sola los pares dañados si los hubiera, dejando el más nuevo',
              '**RESUELTO el 25/8/2026 — el autor de TODA operación de inventario sale de la sesión.** No era solo la lista de preparación: diez puertas del inventario (movimiento manual, fraccionar, corregir, borradores, avanzar, listas, recibir…) aceptaban el `usuarioId` del body. Barrida completa: los DTOs ya no lo declaran (whitelist descarta lo que manden los frontends viejos) y cada handler firma con `sesion.usuarioId` — probado mandando el id de otro usuario: la traza quedó con el de la sesión',
              '**RESUELTO (verificado el 25/8/2026) — el mime se comprueba contra los bytes en los tres lugares.** Gastos (`mimeReal` al subir el adjunto) y las imágenes del sitio web ya verificaban igual que la bandeja de facturas: el pendiente había quedado escrito de antes de la tanda de seguridad del 22/8',
              '**RESUELTO el 25/8/2026 — un borrador no puede recibir mercadería.** `borrador` + `recepcion` se rechaza con motivo («cargalo confirmado, o sin recepción»): antes el stock entraba igual y el documento quedaba para siempre en un limbo sin deuda y sin camino de confirmación. El dashboard nunca mandó borradores, así que no rompe a nadie',
              '**RESUELTO (y con yapa el 25/8/2026) — los archivos de facturas piden sesión y permiso.** El guard global del 22/8 ya lo cerraba (curl sin token: 401); el pendiente era de antes. La yapa: ese cierre había roto las MINIATURAS de la bandeja — un `<img src>` no puede mandar el token — y nadie lo había notado. Ahora el papel se baja con `httpClient.urlProtegida` (fetch con Bearer → URL `blob:` cacheada) y las miniaturas y los «Ver el papel» del alta abren esa URL local',
              '**RESUELTO (verificado el 25/8/2026) — helmet está en `main.ts` desde la tanda del 22/8** (CSP y COEP apagados a propósito: la API no sirve HTML). Cabeceras comprobadas en vivo: `X-Content-Type-Options: nosniff`, `X-Frame-Options`, `Strict-Transport-Security`, sin `X-Powered-By`',
              ...
              '**RESUELTO el 20/8/2026 — el CUIT de la empresa ya es el real** (`23-35678242-9`, razón social `LORENZO LUCAS EMANUEL`, en Sistema › Empresa). Venía con el de prueba `30-71555666-7`, así que la bandeja avisaba "esta factura no es nuestra" en **todas** las facturas — un aviso que siempre suena entrena a ignorar los avisos. Ahora el control sirve para lo que es: detectar la factura que el proveedor emitió a otra razón social. **Lo que sigue con datos de ejemplo son la dirección y el teléfono** (`Av. Siempreviva 742, Pilar` y `11-4000-9999`), y esos se imprimen en el membrete de todo: hay que cargar el domicilio fiscal real.',
              ...
              '**🔴 EL BACKUP AUTOMÁTICO DE PRODUCCIÓN NO EXISTE — verificado el 26/8 mirando Dokploy** (crm-db › Backups vacío: "se necesita al menos 1 provider"). Hasta configurarlo, la única copia de la base es la manual de Sistema › Respaldos — bajarla seguido. **LA GUÍA COMPLETA PARA RETOMARLO**, paso por paso: **(1) EL DESTINO — Cloudflare R2 (recomendado)**: 10 GB gratis y sin costo de descarga (la base pesa ~17 MB: entran AÑOS de copias diarias). Cuenta en **cloudflare.com** → menú **R2 Object Storage** (la primera vez pide habilitar R2, y Cloudflare **exige cargar una tarjeta aunque no se pase del plan gratis** — no cobra nada dentro de los 10 GB) → **Create bucket**, nombre `saboryaroma-respaldos`, ubicación automática → después, en la misma pantalla de R2, **"Manage R2 API Tokens"** → **Create API Token** → permiso **"Object Read & Write"** (alcanza limitarlo a ese bucket) → al crearlo muestra **UNA SOLA VEZ** tres datos que hay que copiar y guardar: **Access Key ID**, **Secret Access Key** y el **endpoint** (una URL del estilo `https://<id-de-cuenta>.r2.cloudflarestorage.com`). **Alternativa equivalente, Backblaze B2** (también ~10 GB gratis, y sin pedir tarjeta): cuenta en backblaze.com → Create Bucket (privado) → App Keys → Add Application Key (limitada al bucket) → da **keyID** (es el access key), **applicationKey** (el secret) y el endpoint S3 que figura en la ficha del bucket (`s3.<región>.backblazeb2.com`). **(2) CARGARLO EN DOKPLOY**: menú **S3 Destinations** (el mismo link que muestra la pantalla vacía de Backups) → **Add Destination** → nombre libre ("R2 respaldos"), y pegar **endpoint**, **Access Key**, **Secret Key**, **bucket** (`saboryaroma-respaldos`) y **región**: en R2 va literalmente `auto` (en B2, la de su endpoint). Probar con el botón de test si lo ofrece, y guardar. **(3) PROGRAMAR EL BACKUP**: volver a **crm-db › Backups → Create Backup** → elegir el destino recién creado, la base **`saboryaromacrm-bvzncl`**, frecuencia diaria con cron **`0 3 * * *`** (OJO: el cron corre en la hora DEL SERVIDOR, que suele estar en UTC — `0 3` UTC son las 00:00 de Argentina, madrugada igual, sirve), **retención 30** copias y un prefijo tipo `crm/` para que el bucket quede ordenado. Guardar: desde ahí corre solo y las copias quedan FUERA del VPS, que es todo el punto. **(4) LA PRUEBA, UNA VEZ**: cuando el primero haya corrido (se ve en la lista de Backups con fecha y tamaño), restaurarlo a una base de PRUEBA (no sobre la real) con el botón Restore de Dokploy, o avisar y lo verificamos juntos — un respaldo nunca probado es una esperanza, no un respaldo. Recién ahí este pendiente se cierra.',
              '**🟠 LOS SALDOS INICIALES DE PROVEEDORES SE CARGAN EL DÍA DEL CORTE — decisión del 26/8, al construir el importador de proveedores.** El importador trae la FICHA y a propósito NO los saldos: un saldo importado hoy queda viejo mañana a la primera factura (en el export del 26/8 la deuda total era ~$222M y cambia todos los días). El plan para cuando el dueño defina el corte con el sistema viejo: **(1)** elegir el día y dejar de cargar compras allá; **(2)** exportar el padrón ESE día y cargar los saldos pendientes de cada proveedor como saldo inicial en el CRM (falta construir ese paso: probablemente un comprobante de ajuste "saldo inicial" por proveedor, sin stock); **(3)** desde ahí, todo por el CRM. Mientras no haya corte, la deuda vieja vive en el sistema viejo y la nueva en el CRM — no sumar las dos.',
              ...
              '**RESUELTO el 25/8/2026 — el flete vuelve a la cadena del sistema anterior.** La pregunta abierta del 19/8 (¿el flete lleva la neutralización del IVA o no?) la contestó el dueño: **tal cual Sistel**, o sea que sí. La condición que lo hace correcto: el flete viene FACTURADO por el transportista (su IVA vuelve como crédito) y el % cargado es el BRUTO. La forma cerrada quedó elegante: base del precio = (mercadería + flete) × ratio, con el MISMO ratio que ya usaba la parte sin factura — y el IVA absorbido es solo de la mercadería, porque el del flete no se pierde. Verificado contra el caso real del Aceite de CARLOS OLIVAA: base $2.863,64/u y góndola $5.197,50 con 50% de markup, los números de Sistel clavados. Con 0% sin factura nada cambia. El detalle completo está en Formato de Compra › Mercadería sin factura.',
              ...
              '**FACTURACIÓN ELECTRÓNICA — HOMOLOGACIÓN ANDANDO (20/8/2026); falta el certificado de PRODUCCIÓN y la fase 8.** El servidor de producción ya tiene su volumen en `/certs`, las cinco variables `ARCA_*` y el certificado de **homologación** instalado desde la pantalla: *Probar conexión* da los tres pasos en verde, con ticket de acceso vigente y **los cinco puntos de venta contestando su último número** (contra el pronóstico: se esperaba que los de producción no existieran en homologación, y contestaron igual). Lo que queda es tramitar el certificado de **producción** —otro trámite, otro certificado, en Administración de Certificados Digitales y no en WSASS— y la fase 8. Fases 3 a 7 hechas: el módulo `arca/` (WSAA, WSFE, armado de A/B con IVA discriminado), el enganche con la venta (numeración desde ARCA, serialización, reserva del número y recuperación), la factura impresa con su QR, **las notas de crédito** (0076) y **el panel de diagnóstico** en Ventas › Configuración. Contra el ARCA de homologación real ya se probó que `FEDummy` responde y que **el WSAA acepta y parsea nuestra firma CMS**. **LO QUE BLOQUEA TODO: el certificado.** El orden es: **(1)** en Dokploy, **montar primero el volumen** en `/certs` (Advanced › Volumes) y recién después cargar las cinco variables `ARCA_*` (documentadas en `.env.example`) apuntando ahí. **El orden importa**: el deploy es automático *On Push* a `main`, y el contenedor se reconstruye entero — una clave generada fuera del volumen desaparece en el próximo merge, en el medio del trámite, y el certificado que ARCA estaba por emitir queda inservible porque la clave no se recupera. Sin esas dos variables **los botones del certificado ni siquiera aparecen** — fue exactamente lo que pasó la primera vez, y el panel lo dijo con nombre y apellido. Dos trampas más del camino, ya resueltas: en Dokploy **guardar no aplica nada, hace falta Redeploy**, y la carpeta `/certs` tiene que existir en la imagen a nombre de `node` (Docker le da a un volumen nuevo el dueño de la carpeta de la imagen; sin ella nacería de `root` y la clave no se podría escribir); **(1b)** tramitar el certificado **desde Ventas › Configuración** —generar la clave y el pedido, subir el pedido a ARCA (WSASS para homologación), **autorizarlo al servicio `wsfe`** (en WSASS es un formulario aparte, *Crear autorización a servicio*: crear el DN NO lo autoriza, y sin eso WSAA contesta **"Computador no autorizado a acceder al servicio"**, que no menciona la autorización por ningún lado). Solo hace falta `wsfe`: el código pide el ticket únicamente para ese servicio. Y pegar el `.crt` que devuelven—; **(2)** apretar **Probar conexión** hasta que los tres pasos den ✔; **sin eso no se sigue**, porque cualquier bug posterior va a parecer del código cuando es un trámite a medias; **(3)** fase 8, el pasaje a producción, con el orden del §12 de la guía (limpiar pendientes ANTES de prender el interruptor, y la primera factura real a mano y mirándola). **PENDIENTES CONCRETOS ANTES DE LA PRIMERA FACTURA**: **CARGAR LOS DATOS FISCALES EN EL SERVIDOR DE PRODUCCIÓN, que están VACÍOS** — el CUIT y la razón social se cargaron el 20/8 en la base de desarrollo (`23-35678242-9` / `LORENZO LUCAS EMANUEL`, con "Sabor y Aroma" como nombre de fantasía en su propio campo) pero **en producción no**, y se vio en la primera factura de homologación: salió **sin el CUIT del emisor** y a nombre del nombre de fantasía. El CAE se pide con `ARCA_CUIT` (el del certificado), así que el comprobante ante ARCA está bien y **el error vive solo en el papel** — que es donde no se mira. Va también el **domicilio fiscal**: la dirección y el teléfono siguen siendo los de ejemplo; y **cargar los cinco puntos de venta en el servidor de producción** (en la base local ya están: Gerencia › Usuarios y roles › Sucursales). **LOS CINCO SON EXCLUSIVOS DE ESTE SISTEMA**: se dieron de alta el 19/8/2026 para él, el sistema viejo factura por otros, y eso hace que los dos puedan convivir durante la transición sin pisarse el correlativo — se puede pasar local por local, y volver atrás si algo sale mal. Como nunca se emitió nada por ellos, el último número autorizado tiene que dar **0** en el panel: si diera otra cosa, algo más está usando ese punto de venta y hay que frenar. **Una decisión del dueño sigue abierta**: si toda venta se factura o el ticket interno sobrevive. Mientras tanto el interruptor de ARCA en Configuración va APAGADO.',
