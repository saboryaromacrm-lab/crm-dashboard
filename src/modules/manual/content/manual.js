        ...
        bloques: [
          {
            ...
            filas: [
              ...
              ['**Proveedores: el flete pagado en efectivo al recibir mercadería, "a cuenta" del proveedor** (17/8/2026, EN STAND BY a pedido tuyo — análisis hecho, nada tocado)', 'Preguntaste cómo cargar el caso de pagarle al flete en efectivo en el momento de la entrega y que eso vaya a cuenta de lo que se le debe al proveedor de la mercadería, con algo así como un recibo. **El mecanismo ya existe**: un pago se puede crear suelto —solo proveedor + importe + medio, sin factura todavía— y si sale en efectivo con turno abierto genera su propio egreso real en el arqueo (eso ES el recibo: fecha, monto, medio, usuario, turno). Y al cargar después la factura de esa entrega, el paso "Cómo se paga" puede **tomar** ese pago ya hecho y descontarlo del total. **El problema que encontré leyendo el código**: ese "tomar el pago" pasa por el mismo candado del modo de cuenta "Por facturas" (el default) — solo acepta el saldo COMPLETO de la factura o una cuota pactada exacta, y el flete nunca va a ser ninguna de las dos cosas. Hoy, para cualquier proveedor en ese modo, **la operación se rechaza** con un mensaje que sugiere pasarlo a modo "libre" — pero eso cambia TODA su cuenta (pierde la disciplina de "factura completa" en cada compra, no solo en el flete), cuando lo que hace falta es una excepción puntual para el pago que se registra en el MISMO acto de cargar la factura (ahí el total ya se valida completo por otro lado: contado + cuotas tienen que sumar el total exacto, así que el candado de la cuota pactada es redundante justo en ese caso). Encontré además que en Cuentas corrientes no hay un botón directo de "pago suelto, sin factura todavía" — el único rodeo hoy es dar de alta un "Compromiso manual" (que es "prometo pagar", al revés de "ya pagué") y pagarlo en el acto. **Nada de esto se tocó** — el dueño lo dejó en stand by. Cuando se retome, el arreglo más liviano es eximir el "tomar pago" del alta de la factura de ese candado puntual (`pagos.module.ts` función `aplicar`, y su llamada desde `comprobantes.module.ts` en el alta del comprobante); de yapa, un botón de "pago a cuenta / sin factura" en Cuentas corrientes evitaría el rodeo del compromiso manual.'],
