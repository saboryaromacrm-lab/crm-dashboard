      ...
      },
      {
        ...
        bloques: [
          ...
          { t: 'ruta', texto: 'Ventas › Configuración › Descuentos con nombre' },
          {
            t: 'nota',
            tono: 'warn',
            texto: 'Al 14/8/2026 están construidos la tabla, el motor, el candado del medio de pago y la pantalla de alta. **Falta el botón "Aplicar descuento" en el punto de venta**, que es por donde se usa — ver Pendientes.',
          },
        ],
      },
    ],
        ...
        bloques: [
          {
            ...
            filas: [
              ['**Descuentos con nombre: falta el botón del punto de venta** (14/8/2026, LO PRIMERO — la función no se puede usar sin esto)', 'La tabla, el motor, el candado del medio de pago y la pantalla de alta están hechos y probados. Lo que falta es la punta por donde se usa: un botón chico **debajo del total** que abra la lista de los que sirven para ESE ticket, y que muestre en gris —con el motivo— los que no (vencido, otra sucursal, ninguna línea de su lista, hace falta un admin). **Las decisiones ya están tomadas, no hay que volver a pensarlas:** el descuento aplicado es **estado del ticket y se re-evalúa en cada cambio**, no un ajuste que se calcula una vez — si se agrega otro producto de esa lista, entra solo; si un renglón cambia de lista, se cae; si se sacan todos, el descuento queda sin efecto y avisa. Y el renglón guarda **dos** descuentos: el que decidió una persona (el del cliente o el que tipeó el vendedor) y el que se muestra, que es el mayor de los dos. **A la API viaja el primero**, nunca el combinado: mandar el 25% autorizado como si fuera puesto a mano lo haría rebotar contra el tope del vendedor **antes** de que el servidor llegue a aplicar el nombrado. El trabajo que queda: las dos acciones del motor que todavía escriben el descuento visible en vez de la base, la acción de aplicar/quitar, cargar el catálogo al abrir la caja, mandar los ids al guardar, y el desplegable. Después, la tanda de pruebas de los cruces que pierden plata (gana el mayor, la oferta excluida, dos de la misma lista, otra sucursal, el que vence hoy a las 23, el pago mixto, la cajera sin permiso, y un porcentaje mandado a mano que no cambia nada)'],
