        ...
        bloques: [
          {
            ...
            items: [
              ...
              '**🔴 EL BACKUP AUTOMÁTICO DE PRODUCCIÓN NO EXISTE — verificado el 26/8 mirando Dokploy**: crm-db › Backups está vacío ("se necesita al menos 1 provider"). Hasta que se configure, la única copia de la base es la que se baje a mano de Sistema › Respaldos (bajarla seguido). Los pasos, ya charlados: **(1)** crear un bucket gratis en Cloudflare R2 (10 GB — con la base de 17 MB entran años de copias diarias) y su API token de lectura/escritura; **(2)** cargarlo en Dokploy › S3 Destinations (endpoint + access key + secret + bucket, región `auto`); **(3)** en crm-db › Backups programar el diario (cron `0 3 * * *`, retención 30); **(4)** cuando corra el primero, restaurarlo UNA vez a una base de prueba — un respaldo nunca probado es una esperanza, no un respaldo.',
