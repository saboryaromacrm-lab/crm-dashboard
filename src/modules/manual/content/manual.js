        ...
        bloques: [
          {
            t: 'lista',
            items: [
              '**Fotos de la tienda en LOTE — analizado el 25/8/2026, aprobación pendiente.** Hoy es de a una (fila → Subir → modal → Usar), ~5 acciones por producto. Plan en dos piezas, cero cambios de API (el endpoint por producto ya valida todo): **(A)** botón "Subir fotos en lote" en Web › Productos — se arrastran decenas de archivos y se emparejan por el NOMBRE del archivo: código de barras exacto, código propio, o nombre normalizado; pantalla de revisión (emparejado / dudoso / sin destino, los sueltos se asignan con un buscador por fila) y subida con el molde de siempre + barra de progreso. **(B)** modo ráfaga: recorre SOLO los productos sin foto, una foto arrastrada = guardada y pasa al siguiente. Extras detectados: falta filtro/contador "sin foto" en la lista; las fotos viven en la base (base64, ~20 MB con 200 productos — bien hoy, revisar si crece a miles); UNA foto por producto (índice único: una galería sería migración); la tienda solo vende el entero, así que nada choca con el fraccionado.',
