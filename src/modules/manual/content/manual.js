        ...
        bloques: [
          {
            ...
            items: [
              ...
              '**FACTURACIÓN ELECTRÓNICA — falta el certificado (fases 6 a 8).** Fases 3, 4 y 5 hechas: el módulo `arca/` (WSAA, WSFE, armado de A/B con IVA discriminado), el enganche con la venta (numeración desde ARCA, serialización, reserva del número y recuperación) y la factura impresa con su QR. Contra el ARCA de homologación real ya se probó que `FEDummy` responde y que **el WSAA acepta y parsea nuestra firma CMS**. **LO QUE BLOQUEA TODO: el certificado.** Después: **(1)** subirlo al servidor —volumen montado, fuera de la imagen— y cargar las cinco variables `ARCA_*` (documentadas en `.env.example`); **(2)** correr `node dist/arca/probar.js` y que dé OK; **sin eso no se sigue**, porque cualquier bug posterior va a parecer del código cuando es un trámite a medias; **(3)** fase 6, el panel de diagnóstico; **(4)** fase 7, las notas de crédito — acá SÍ hacen falta: una venta con CAE **no se puede anular nunca**, y hoy `anular()` no lo sabe; **(5)** fase 8, el pasaje a producción, con el orden del §12 de la guía (limpiar pendientes ANTES de prender el interruptor, y la primera factura real a mano y mirándola). **Tres decisiones del dueño siguen abiertas**: si toda venta se factura o el ticket interno sobrevive, si se hacen notas de crédito, y los datos fiscales reales (hoy Sistema › Empresa tiene datos de ejemplo — "Av. Siempreviva 742"). Mientras tanto el interruptor de ARCA en Configuración va APAGADO.',
