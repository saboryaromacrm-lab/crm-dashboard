        ...
        bloques: [
          {
            ...
            filas: [
              ...
              ['**Dos cajeras en un local, un solo turno de caja** (21/8/2026 — **RESUELTO EL MISMO DÍA: queda como está**, probado por el dueño)', '**La respuesta fue que en el local hay UN solo cajón físico**, compartido por las dos cajeras. Con eso el modelo actual es el correcto y no hay nada que construir: un cajón, un turno, un arqueo. El dueño lo probó entrando con **dos cajeras a la vez en la misma sucursal** y haciendo ventas con las dos: **el listado de ventas muestra el nombre de cada una**, porque `ventas.usuarioId` guarda quién vendió aunque el turno sea compartido. O sea que se sabe qué vendió cada una, que era lo que importaba. Lo único que sigue sin poder responderse es **de quién fue un faltante**, y eso es inherente a compartir un cajón: ningún sistema lo puede saber si las dos meten la mano en el mismo lugar. **Si algún día se pone un segundo cajón, esto se reabre** y ahí sí el turno tendría que pasar a ser por terminal. Queda anotado el análisis original por si eso pasa: la invariante está escrita en el encabezado de `caja.module.ts` y `abrir` la hace cumplir — **una sola sesión abierta por sucursal**. Eso se leyó al principio como un problema —dos cajeras cobrando a la vez en Distribuidora y en Fontana, un solo turno— **suponiendo dos cajones**: ahí el arqueo del cierre habría sumado dos cajones en uno, el `montoInicial` habría sido uno solo para dos fondos, y un faltante no habría tenido dueño. **Con un solo cajón nada de eso aplica**, y la segunda cajera entrando sobre el turno ya abierto es el comportamiento correcto, no un síntoma. Si mañana hay dos cajones, lo que hace falta ya está a mitad de camino: `ventas.usuarioId` y `cajaMovimientos.usuarioId` existen, así que **la plata ya se puede atribuir por persona** y lo único que faltaría es que el TURNO sea por cajón — la forma natural de nombrar el cajón es la **terminal** de la entrada de arriba. **Sería el único de los cambios propuestos que toca datos existentes** (habría que migrar los turnos abiertos de producción). **Lo que NO se resuelve con un cajón compartido**, y sigue en pie: las dos cajeras facturan por el **mismo punto de venta**. Para ARCA eso está perfecto (un punto de venta admite varias terminales y la numeración la lleva ARCA), pero del lado nuestro es **exactamente la carrera del `FOR UPDATE` que falta** — con una cajera por local era teórica, con dos cobrando en simultáneo es cuestión de tiempo que dos ventas se lleven el mismo número, y eso es independiente de cuántos cajones haya'],
              ['**Los tres caminos que emiten comprobantes no toman lock sobre la venta** (21/8/2026) — **LO PRIMERO DE LA LISTA**: bloquea el certificado de producción y conviene arreglarlo ANTES de la ronda de pruebas', 'Lo encontró la auditoría del 21/8. `confirmar()`, `facturarAhora()` y `notaCredito()` leen **fuera de la transacción** el estado que los habilita (que sea borrador, que esté pendiente, cuánto queda por acreditar) y no toman la fila de `ventas` con `FOR UPDATE`. Dos pedidos en paralelo —dos pestañas, un doble clic sobre un botón que tarda hasta 15 s, o dos cajas— leen los dos el mismo estado y pasan los dos: **dos CAE para la misma venta**, o una nota de crédito por el 200%. En homologación eso deja basura; en producción deja comprobantes fiscales huérfanos que solo se corrigen con otra nota de crédito y quedan en el libro de IVA. El patrón para arreglarlo **ya está en el mismo archivo**, aplicado al turno de caja (`.for(\'update\')`): es tomar la fila al abrir la transacción y revalidar adentro lo que hoy se valida afuera. **21/8 a la tarde: pasó de probable a inminente.** Revisando el login salió que en **Distribuidora y en Fontana hay DOS cajeras cobrando a la vez**, y las dos facturan por el **mismo punto de venta** de ARCA. El "dos cajas" de arriba dejó de ser un escenario de laboratorio: es la operación normal de dos de los cinco locales, todos los días — y **el cajón compartido no lo cambia**, porque la carrera es por el número del comprobante y no por la plata. **Por eso conviene arreglarlo antes de la ronda de pruebas y no después**: si durante las pruebas dos cajeras se llevan el mismo número, el síntoma va a parecer cualquier otra cosa y se van a perder horas buscándolo en el lugar equivocado'],
