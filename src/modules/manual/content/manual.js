        ...
        bloques: [
          {
            ...
            filas: [
              ...
              ['**Dos cajeras en un local, un solo turno de caja** (21/8/2026, apareció analizando lo del login — SIN DECIDIR, va junto con lo de arriba)', 'La invariante está escrita en el encabezado de `caja.module.ts` y `abrir` la hace cumplir: **una sola sesión abierta por sucursal**. Pero el dueño confirmó el 21/8 que en **Distribuidora hay dos cajeras cobrando a la vez, y en Fontana también**. O sea: **dos cajones físicos y un solo turno en el sistema**. Lo que está pasando hoy: el arqueo del cierre **suma los dos cajones en uno**, así que si falta plata no hay forma de saber de cuál cajón faltó — el control de caja pierde exactamente la función para la que existe; el `montoInicial` es uno solo cuando en realidad son dos fondos iniciales distintos; `cajaSesiones.usuarioId` guarda quién **abrió**, así que el día entero queda a nombre de una sola de las dos; y la segunda que llega a abrir rebota con "Ya hay un turno abierto", con lo cual o no abre o trabajan las dos sobre el mismo turno (que es lo que evidentemente pasa). **Lo bueno: el dato para arreglarlo ya existe.** `ventas.usuarioId` está, y `cajaMovimientos.usuarioId` también — **la plata ya se puede atribuir por persona**; lo que falta es que el TURNO sea por cajón para poder arquear cada uno por separado. La forma natural de nombrar el cajón es la **terminal** de la entrada de arriba: registrada la PC, el turno pasa a ser por terminal en vez de por sucursal y salen dos turnos, dos fondos iniciales y dos arqueos independientes, cada uno a nombre de quien lo trabajó. **Es el único de los cambios propuestos que toca datos existentes** (hay que migrar los turnos abiertos de producción), así que va al final y con cuidado. Y ojo con el cruce fiscal: las dos cajeras de un mismo local facturan por el **mismo punto de venta**. Para ARCA eso está perfecto (un punto de venta admite varias terminales y la numeración la lleva ARCA), pero del lado nuestro es **exactamente la carrera del `FOR UPDATE` que falta** — con una cajera por local era teórica, con dos en simultáneo es cuestión de tiempo que dos ventas se lleven el mismo número'],
              ['**Gerencia: la sucursal del turno la elige el propio empleado** (14/8/2026, el análisis viejo — lo REEMPLAZA la entrada de arriba, se deja por el historial)', 'En el login solo se valida que la sucursal **exista**: no hay ninguna tabla que diga a qué sucursales pertenece cada usuario. Carla, cajera de Express 3, sale y vuelve a entrar eligiendo "Fontana", y a partir de ahí **para el servidor ES de Fontana**: cierra ese turno con el efectivo que ella declare, le carga movimientos de caja y le ajusta el stock. Todo firmado con su nombre pero en el local de otro. Esto vacía buena parte de `soloSuSucursal`, que es el candado que se puso en Ventas y en Almacén — hoy es una cerradura con la llave puesta del lado del cajero. **El dueño ya decidió lo que faltaba saber (14/8): las cajeras SÍ rotan entre sucursales**, así que es una relación de muchos a muchos —tabla `usuario_sucursales`— y no un campo en el usuario. Lo que queda por construir: la migración, asignar las sucursales de cada uno en Gerencia › Usuarios, y validar la elección en el login (el jefe elige libremente; el cajero, entre las suyas). Ojo con el orden al aplicarlo: si la tabla nace vacía, nadie puede entrar a ningún lado — el alta tiene que sembrar a cada usuario con las sucursales que hoy usa, o el login tiene que tratar "sin ninguna asignada" como "todas" hasta que se carguen'],
              ...
              ['**CUIT de los proveedores**', 'El reconocimiento automático de la bandeja va **por CUIT**: los proveedores que no lo tengan cargado llegan sin proveedor y hay que elegirlo a mano. (El de la empresa ya está: se cargó el real el 20/8/2026 y el falso aviso "no es nuestra" se apagó.)'],
