    ...
    temas: [
      {
        ...
        bloques: [
          ...
          },
          {
            ...
            filas: [
              ['**18/9/2026**', '**EL BULTO SE VE Y SE ELIGE EN LA CAJA: antes solo se llegaba escaneando** (pedido del dueño: &ldquo;al seleccionar un producto que se vende por bulto, que salga con la cantidad de ese bulto — hoy sale por Unidad&rdquo;). Un artículo que se vende de a 12 estaba en el POS **una sola vez y decía &ldquo;Unidad&rdquo;**, porque ese texto sale del TIPO del producto (entero o granel) y el bulto no es un tipo: es una fila del **formato de venta** con `unidades: 12`. Al bulto se llegaba **por un solo camino, el código de barras de la caja** — y si la caja no tiene código propio, o el cajero busca por nombre, no había forma: ponía 1, cobraba precio de mostrador, y la caja de doce salía como una botella suelta. **Ahora cada formato de a N aparece como su propia opción**, debajo de la unidad: *&ldquo;Bulto × 12 · Mayorista 1&rdquo;*, con **cuántos bultos enteros hay en el depósito** (&ldquo;30 u. · 2 bultos&rdquo;, para no restar de memoria) y **el precio del bulto cerrado**, que es lo que pregunta el que se lleva la caja. Elegirlo carga las 12 y **fija la lista** — llevarse la caja ES elegir el precio mayorista. **No se inventó ningún mecanismo**: se marca igual que un escaneo (`_escaneoUnidades` + `_escaneoListaId`), o sea el mismo camino que ya existía y ya estaba probado; lo único que faltaba era hacerlo visible. **La unidad va primero y siempre**, porque Enter agrega el primero de la lista y la venta de todos los días es suelta: elegir el bulto es un acto deliberado. **Y el escaneo se emparejó**: escaneando el EAN de la caja, la fila decía &ldquo;Unidad · $1.815&rdquo; mientras por dentro cargaba 12 al precio mayorista — hacía lo correcto y se explicaba mal. Ahora las dos puertas muestran lo mismo. **La que NO cambió es la del ticket**, y es a propósito: el renglón sigue contando **UNIDADES** (12, no &ldquo;1 bulto&rdquo;), porque doce sueltas más una caja de doce tienen que sumar en un solo renglón de 24 — y el stock, los totales y las ofertas cuentan unidades. Un renglón que dijera &ldquo;Bulto × 12&rdquo; mentiría apenas alguien agregue una más. **Del lado de la API viaja un dato nuevo, `precioFormato`**: el precio del bulto ENTERO, y no el unitario multiplicado. Con un bulto fijado en $10.000, el unitario derivado es $833,33 y multiplicarlo por 12 en la pantalla daría **$9.999,96** — un cartel contradiciendo al que lo puso. **Pendiente conocido, anterior a esto**: el ticket trabaja en precios unitarios de dos decimales, así que ese mismo bulto de $10.000 cierra en **$10.000,07**. No lo trajo este cambio (escanear la caja daba lo mismo antes), pero ahora se ven los dos números juntos. Probado con 5 casos automáticos y verificado en pantalla de punta a punta con datos ZZ_, después borrados.'],
