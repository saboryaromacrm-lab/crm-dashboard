        ...
        bloques: [
          {
            ...
            filas: [
              ...
              ['**La cajera elige a mano en qué sucursal entra, y el sistema le propone la equivocada** (21/8/2026, ANALIZADO A FONDO — **EN STAND BY**: el dueño lo va a pensar)', '**El error no lo empieza la cajera: lo empieza el default.** `LoginPage.jsx` preselecciona `sucs[0]`, y como `/auth/opciones` ordena por id eso es **siempre la Distribuidora**. La cajera de Express 2 que no toca ese campo entra en la Distribuidora sin haber elegido nada. La pantalla de confirmación existe y está bien pensada, pero **dice exactamente lo mismo cuando está bien que cuando está mal**, y una confirmación que siempre dice lo mismo se vuelve invisible a la tercera vez. **CONTRA LA INTUICIÓN: cerrar caja NO lo detecta.** Vende en el local equivocado, la plata entra a la caja de ese local, cuenta su cajón al cerrar y **la diferencia da $0**. El arqueo es internamente coherente, así que nunca puede delatarlo. Lo que queda roto es silencioso: stock descontado de un local que no vendió, el suyo inflado, y plata física en Express 2 anotada como plata de la Distribuidora. No aparece hasta un control de stock semanas después. **Dónde SÍ se detecta hoy, de casualidad**: `abrir` no deja dos turnos en la misma sucursal. Si el cajero real ya abrió, la otra rebota — pero con el mensaje "Ya hay un turno de caja abierto en esta sucursal", que no sugiere en ningún momento que el problema sea DÓNDE está parada. Y tiene un modo de falla peor: si la equivocada abre primero, el cajero real rebota y la reacción natural es seguir sobre el turno ya abierto. **Lo que cambió el 20/8 y sube esto de molesto a caro: ARCA.** `sucursales.puntoVenta` tiene numeración correlativa propia y `sucursales.direccion` es el domicilio que va impreso. Facturar desde la sucursal equivocada **consume un número del correlativo que no era** (ese número no vuelve), imprime el domicilio de otro local (RG 1415) y **tiene CAE: no se borra**. Solo se arregla con nota de crédito y refacturar. Asimetría que define todo: un ticket interno mal se arregla anulando y rehaciendo, una factura no — **por eso la prevención tiene que estar en el login y no en un reporte posterior**. **LAS CUATRO OPCIONES, evaluadas el 21/8.** (1) **Leer el nombre del equipo — IMPOSIBLE.** Es una web en un navegador: no existe API de JavaScript que devuelva hostname, serial ni MAC. El navegador lo prohíbe a propósito (sería un identificador de rastreo). No hay truco. (2) **Geolocalización — DESCARTADA.** Pide permiso que se puede rechazar; en PC de escritorio no hay GPS y la posición sale de IP/WiFi con precisión de cientos de metros a kilómetros. Fontana contra Resistencia tal vez lo distinga, **Distribuidora contra Express 1/2/3 dentro de Resistencia casi seguro que no** — y esos son justo los que se confunden. Además falla en la dirección peligrosa: un día no resuelve y la cajera no puede entrar. (3) **IP pública → sucursal — DESCARTADA el 21/8, y por un motivo que no estaba a la vista: cuando se corta internet las cajeras siguen trabajando con los DATOS DEL CELULAR.** Ahí la IP pasa a ser la del operador móvil: cambia sola, es distinta según de qué celular se colgaron, y dos locales sobre el mismo operador pueden verse idénticos. Peor todavía es lo que haría el aviso: **saltaría justo los días que se cae internet**, o sea los días de más quilombo, y un aviso que suena cuando no corresponde enseña a ignorar todos los avisos — el mismo error que se corrigió el 20/8 con los dos falsos avisos del CUIT. Se anota igual el análisis por si algún día las conexiones se estabilizan: cada local tiene su conexión; la API **ya lee `req.ip`** en el login y el tema de la falsificación ya está resuelto con el `trust proxy` de `main.ts`. Ventajas únicas: vive en el servidor (**es la única señal que el navegador no puede mentir**), cero configuración en las máquinas, cubre las dos cajas y la tablet y el celular, y **se puede auto-aprender** (guardar de qué IP entró cada sesión y qué sucursal eligió; después de N logins coherentes esa IP ES ese local, con una pantalla para confirmar o corregir). Tres trampas a despejar antes: IP dinámica (se re-enseña sola, pero el primer login queda sin red — se mitiga mirando el /24, que cambia mucho menos), **4G/CGNAT** (comparte IP pública con otros clientes: dos locales con 4G del mismo operador pueden verse idénticos) y enlace o VPN compartida entre locales. (4) **Registrar la TERMINAL — LA MEJOR, y es la recomendación.** Tabla `terminales` (nombre, sucursalId, token, activa, últimoUso). El jefe registra cada equipo **una sola vez** desde esa PC: elige sucursal, le pone nombre, y queda un token en el `localStorage` de ese navegador. Desde ahí **el login de esa PC ya no pregunta la sucursal**: la muestra fija ("Caja 2 · Distribuidora") y la cajera solo elige quién es y pone su clave. La diferencia de fondo con todo lo demás: **no avisa mejor, saca la pregunta** — y sin decisión no hay error posible. La IP queda como **verificación cruzada** (si esa terminal aparece desde una IP que nunca fue de ese local, avisa: cubre la notebook que alguien se llevó). Si se borra el `localStorage` se re-registra y mientras tanto el login vuelve a preguntar con el aviso de IP: molestia ocasional, no caída. Y de yapa, **la terminal es lo que le da nombre al cajón** y destraba el problema de las dos cajeras (entrada aparte). **Por qué NO conviene la solución obvia** de asignarle la sucursal a cada usuario: **las cajeras rotan** (confirmado por el dueño el 14/8), así que habría que reasignarlas a mano todos los días. Se estaría atando el dato al elemento que se mueve en vez de al que está clavado — **la PC de Express 2 está siempre en Express 2**. Si igual se quiere, que **avise y nunca trabe**: un candado que hay que abrir todos los días termina abierto para todos. **LO QUE FALTA PARA DECIDIR** (ya casi nada, se contestó el 21/8): la conexión de cada local **es distinta y además inestable** —en Distribuidora hay un repetidor y cuando se corta se sigue con datos del celular—, lo que mató la opción 3 y de paso **deja a la 4 como la única en pie**. Y le juega a favor: **el token de la terminal vive en el navegador, no en la red**, así que funciona igual con fibra, con el repetidor o colgado del celular de la cajera. Queda confirmar **cuántos puestos hay** (Distribuidora 2, Fontana 2, y Express 1/2/3 uno cada uno = 7 terminales, que se registran en un rato una sola vez). **Orden propuesto cuando se retome** (quedó en dos pasos, no cuatro): 1) el `FOR UPDATE` de los tres caminos de emisión (bloqueante, y las dos cajeras lo volvieron inminente), 2) **terminales + login sin pregunta de sucursal**, que es todo lo que queda en pie. La opción 3 murió por los datos del celular, y la 4 original —caja por terminal— **se cayó sola cuando el dueño confirmó que el cajón es uno solo** (ver la entrada de las dos cajeras)'],
