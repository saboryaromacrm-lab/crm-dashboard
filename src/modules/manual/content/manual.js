        ...
        bloques: [
          {
            ...
            items: [
              ...
              '**FACTURACIÓN ELECTRÓNICA — falta el certificado y el enganche (fases 4 a 8).** El módulo `arca/` ya está construido y compila (fase 3, 19/8): configuración con interruptor, WSAA con firma CMS y cache de tres niveles, WSFE con las cuatro llamadas, y el armado de Factura A/B con el IVA discriminado por alícuota. `FEDummy` ya responde OK contra homologación. **Lo que falta: (1)** el certificado de homologación y su autorización a `wsfe` y `ws_sr_constancia_inscripcion`; **(2)** correr `node dist/arca/probar.js` y que dé OK — sin eso no se sigue; **(3)** la fase 4, enchufar `solicitarCae()` de ventas al módulo, con la numeración viniendo de ARCA (hoy es un contador local `max+1` y **no puede competir con la de ARCA**) y su candado de concurrencia; **(4)** el QR fiscal en la impresión; **(5)** el panel de diagnóstico; **(6)** las notas de crédito, que acá SÍ hacen falta (una venta con CAE no se puede anular nunca). **Tres decisiones del dueño siguen abiertas**: si toda venta se factura o el ticket interno sobrevive, si se hacen notas de crédito, y los datos fiscales reales (hoy Sistema › Empresa tiene datos de ejemplo — "Av. Siempreviva 742"). Mientras tanto el interruptor de ARCA en Configuración va APAGADO.',
